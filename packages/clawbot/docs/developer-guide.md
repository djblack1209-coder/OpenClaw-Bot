...
> 项目位置: `~/Desktop/OpenClaw Bot/clawbot`  
> 版本: v5.0 (Mixin 架构版)  
> 最后更新: 2026-02-24
...
- 6 个 AI 模型（Qwen-3-235B、GPT-OSS-120B、Claude Sonnet/Haiku/Opus、DeepSeek V3）协作决策
- IBKR 盈透证券实盘/模拟交易
- 技术分析信号引擎（RSI/MACD/EMA/布林带/ATR）
- AI 团队投票 → 风控校验 → 自动下单完整流水线
- 仓位管理、止损止盈、每日亏损限额、熔断保护
- 群聊协作（多 Bot 在同一群组，智能路由消息）
- 对话持久化（SQLite）、行情缓存、弹性 HTTP 客户端
...
```bash
cd ~/Desktop/OpenClaw\ Bot/clawbot

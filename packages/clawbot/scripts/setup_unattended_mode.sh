...
# 自动定位项目根目录（脚本位于 packages/clawbot/scripts/ 下）
ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
LOG_DIR="$ROOT/packages/clawbot/logs"
...
chmod +x "$ROOT/packages/clawbot/scripts/openclaw_keepawake.sh"
chmod +x "$ROOT/packages/clawbot/scripts/bootstrap_browser_session.py"
cp "$ROOT/packages/clawbot/scripts/openclaw_keepawake.sh" "$RUNTIME_BIN_DIR/openclaw_keepawake.sh"
cp "$ROOT/packages/clawbot/scripts/bootstrap_browser_session.py" "$RUNTIME_BIN_DIR/bootstrap_browser_session.py"
...
python3 "$ROOT/packages/clawbot/scripts/bootstrap_browser_session.py"

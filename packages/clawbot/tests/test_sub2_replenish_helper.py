...
def test_parser_accepts_fixed_format_and_public_state_is_redacted():
    credential = parse_seller_payload(SELLER_LINE)[0]
...
def test_plan_matching_and_duplicate_detection_do_not_guess_channel():
    groups = [
    ...
    assert [group["id"] for group in matching_plan_groups(groups, "plus")] == [1, 2]
    assert [group["id"] for group in matching_plan_groups(groups, "pro")] == [3]
    assert matching_plan_groups(groups, "team") == []
    assert [group["id"] for group in manual_openai_group_options(groups)] == [1, 2, 3]
...
def test_dry_run_ui_requires_same_origin_session_and_never_persists_secrets(tmp_path, monkeypatch):
    ...
    with TestClient(app, base_url=ORIGIN) as client:
        ...
        parsed = client.post("/api/parse", headers=headers, json={"raw": SELLER_LINE})

...
class OpenClawBrain:
                    ...
                    if free_pool:
                        async with api_limiter("llm"):
                            resp = await free_pool.acompletion(
                                ...
                                messages=[
                                    {"role": "system", "content":
                                     "你是 OpenClaw，一个友好的数字生命助手。简洁地用中文回答。"},
            ...
            if free_pool:
                ...
                async with api_limiter("llm"):
                    resp = await free_pool.acompletion(
                        ...
                        system_prompt="你是OpenClaw投资总监。基于研究员、技术分析、量化和风控的分析结果，做出最终决策。用JSON格式回复: {\"decision\": \"buy/sell/hold\", \"confidence\": 0.0-1.0, \"reasoning\": \"理由\", \"position_pct\": 0-20}",
                        temperature=0.3,
                        max_tokens=500,
                    )
    ...
    async def _exec_social_publish(self, params: Dict) -> Dict:
        ...
        try:
            if platform in ("x", "twitter"):
                ...
                result = await run_social_worker(f"publish_{platform}", {"content": draft})
                    ...
                    if free_pool:
                        async with api_limiter("llm"):
                            resp = await free_pool.acompletion(
                                ...
                                messages=[
                                    {"role": "system", "content": (
                                        "你是专业购物比价助手。根据搜索结果提供各平台价格对比和购买建议。"
    ...
    async def _exec_llm_query(self, params: Dict) -> Dict:
            ...
            if free_pool:
                async with api_limiter("llm"):
                    resp = await free_pool.acompletion(
                        ...
                        system_prompt="你是 OpenClaw，一个全能AI助手。简洁回答问题。",
                        temperature=0.7,
                        max_tokens=1000,
                    )
                answer = resp.choices[0].message.content
        ...
        return {"source": "llm_fallback", "answer": "抱歉，暂时无法回答此问题。"}
            ...
            if any(kw in task_desc for kw in ["import ", "def ", "print(", "for ", "class "]):
                ...
                if free_pool:
                    resp = await free_pool.acompletion(
                        ...
                        system_prompt="你是Python代码生成器。只返回可执行的Python代码，不要解释。用```python代码块包裹。",

...
_CATEGORY_KEYWORDS = {
    ...
    "购物": ["淘宝", "京东", "拼多多", "购物", "衣服", "鞋", "包", "化妆品", "护肤", "日用品", "超市"],
    "通信": ["话费", "流量", "宽带", "网费", "手机", "充值"],
    "娱乐": ["电影", "游戏", "KTV", "唱歌", "旅游", "门票", "演出", "健身", "运动", "会员", "订阅", "视频"],
    ...
    "美容": ["理发", "美甲", "美容", "美发", "染发", "烫发", "面膜", "spa", "按摩", "护理", "美睫", "纹眉"],
    "保险": ["保险", "社保", "医保", "车险", "意外险", "重疾险", "寿险", "公积金", "保费"],
    "人情": ["礼金", "份子钱", "随份子", "请客", "送礼", "婚礼", "满月", "生日礼物", "孝敬", "压岁钱"],
    "烟酒": ["烟", "酒", "香烟", "啤酒", "白酒", "红酒", "雪茄"],
    "工资": ["工资", "薪资", "薪水", "月薪", "底薪", "基本工资"],
    "兼职": ["兼职", "外快", "副业", "零工", "打工"],
    "理财": ["利息", "分红", "投资收益", "股息", "基金收益", "理财收益", "回报", "返利"],
    "转账": ["转账", "红包", "收款", "借款", "还款"],
}
...
def add_expense(
    user_id: int, amount: float, note: str = "", category: str = "其他", chat_id: int = 0, db_path=None
) -> dict:
    """记录一笔开支"""
    # 金额验证
    ...
    try:
        with get_conn(db_path) as conn:
            conn.execute(
                "INSERT INTO expenses (user_id, chat_id, category, amount, note, type) VALUES (?,?,?,?,?,'expense')",
...
def add_income(
    ...
    try:
        with get_conn(db_path) as conn:
            conn.execute(
                "INSERT INTO expenses (user_id, chat_id, category, amount, note, type) VALUES (?,?,?,?,?,'income')",
                (user_id, chat_id, category, amount, note),
            )
        return {"success": True, "amount": amount, "category": category, "note": note}
    except Exception as e:
...
def set_monthly_budget(user_id, budget: float, db_path=None) -> dict:
    ...
    try:
        ...
        with get_conn(db_path) as conn:
            conn.execute(
                "INSERT OR REPLACE INTO budgets (user_id, monthly_budget, updated_at) VALUES (?, ?, ?)",
...
def get_monthly_summary(user_id: int, year_month: str = None, db_path=None) -> dict:
    ...
    try:
        with get_conn(db_path) as conn:
            ...
            # 总收入
            row = conn.execute(
                "SELECT COALESCE(SUM(amount), 0) FROM expenses WHERE user_id=? AND ts>=? AND ts<=? AND type='income'",
                (user_id, ts_start, ts_end),
            ).fetchone()
...
def check_budget_alert(user_id, db_path=None) -> tuple:
    ...
    if pct > 100:
        ...
        msg = f"✅ {month} 预算健康\n预算: ¥{budget:,.0f} | 已花: ¥{spent:,.0f} ({pct:.1f}%)\n剩余: ¥{remaining:,.0f}"
        return (False, msg)
...
def delete_last_expense(user_id: int, chat_id: int = 0, db_path=None) -> bool:
    ...
    try:
        with get_conn(db_path) as conn:
            if chat_id > 0:
                conn.execute(
                    "DELETE FROM expenses WHERE id = (SELECT MAX(id) FROM expenses WHERE user_id=? AND chat_id=?)",
                    (user_id, chat_id),
                )
            else:
                conn.execute(
...
def format_monthly_report(summary: dict) -> str:
    ...
    if by_category:
        ...
        for cat in by_category[:8]:
            ...
            lines.append(f"  {emoji} {cat['category']}: ¥{cat['amount']:,.0f} ({cat['pct']:.1f}%)")
...
def remove_bill_account(account_id, user_id, db_path=None) -> bool:
    ...
    try:
        with get_conn(db_path) as conn:
            cursor = conn.execute(
                "UPDATE bill_accounts SET status='deleted' WHERE id=? AND user_id=? AND status='active'",
...
def get_discount_suggestions(account_type: str, db_path=None) -> str:
    ...
    try:
        with get_conn(db_path) as conn:
            row = conn.execute(
                "SELECT discount_info, updated_at FROM bill_discount_cache WHERE account_type=?",
                (account_type,),
            ).fetchone()

...
class SmartMemoryPipeline:
    ...
    async def on_message(self, chat_id: int, user_id: int, role: str, content: str, bot_id: str = ""):
        ...
        if role == "user":
            ...
            _pref_t.add_done_callback(
                lambda t: t.exception() and logger.debug("偏好检测后台任务异常: %s", t.exception())
            )

        # 到达提取阈值 — 异步触发，不阻塞（跳过已在提取中的 chat + 全局限流）
        now_ts = time.time()
        if (
            turn % self.extract_interval == 0
            and self.llm_fn
            ...
            _t.add_done_callback(
                lambda t: (
                    ...
                    t.exception() and logger.debug("事实提取后台任务异常: %s", t.exception()),
                )
            )
        ...
        if turn % self.profile_interval == 0 and self.llm_fn:
            ...
            _t2.add_done_callback(
                lambda t: t.exception() and logger.debug("用户画像更新后台任务异常: %s", t.exception())
            )
        ...
        try:
            ...
            if self.memory:
                ...
                if self.llm_fn:
                    ...
                    _profile_t.add_done_callback(
                        lambda t: t.exception() and logger.debug("画像即时更新异常: %s", t.exception())
                    )
    ...
    async def _extract_and_store(self, chat_id: int, user_id: int, bot_id: str):
        ...
        try:
            ...
            # 阶段1: 提取事实
            conversation = "\n".join(
                f"{'用户' if m['role'] == 'user' else 'AI'}: {m['content'][:300]}" for m in messages[-10:]
            )
    ...
    async def _resolve_and_store(self, fact: str, chat_id: int, user_id: int, bot_id: str):
        ...
        try:
            ...
            # 格式化已有记忆
            existing_text = "\n".join(f"- id: {m.get('key', '')}, 内容: {m.get('value', '')[:100]}" for m in existing)
    ...
    async def _update_user_profile(self, chat_id: int, user_id: int):
        ...
        try:
            ...
            memories_text = "\n".join(f"- {m.get('value', '')[:150]}" for m in all_memories)
    ...
    CONSOLIDATION_PROMPT = """你是记忆整合器。将下面的碎片化记忆合并为精炼的摘要。
...
```"""
    ...
    async def consolidate(self, chat_id: int = None, category: str = None, min_count: int = 15):
        ...
        - 当同类记忆超过 min_count 条时触发
        - LLM 合并重复/相似记忆
        - 删除被合并的旧记忆，写入新的整合摘要

        参数:
            chat_id: 指定聊天ID（None=全局）
            category: 指定分类（None=所有分类）
            min_count: 触发整合的最小记忆数量
        """
        if not self.llm_fn:
            logger.debug("[SmartMemory] 记忆整合跳过: LLM 函数未设置")
        ...
        try:
            ...
            if chat_id is not None:
                memories = [m for m in memories if m.get("chat_id") == chat_id or m.get("chat_id") is None]

            if len(memories) < min_count:
                return {"consolidated": 0, "removed": 0, "reason": f"记忆数量{len(memories)}未达到整合阈值{min_count}"}

            # 准备 LLM 输入
            memory_texts = []
            for i, m in enumerate(memories[:50]):  # 最多处理50条，控制 token 成本
                key = m.get("key", f"mem_{i}")
                value = str(m.get("value", ""))[:200]
                cat = m.get("category", "general")
                memory_texts.append(f"[{i + 1}] key={key} | category={cat} | {value}")

            prompt = self.CONSOLIDATION_PROMPT.format(count=len(memory_texts), memories="\n".join(memory_texts))

            # 调用 LLM 整合
            response = await self.llm_fn(prompt)
            if not response:
                return {"consolidated": 0, "removed": 0, "error": "LLM 无响应"}
            ...
            if not isinstance(data, dict):
                return {"consolidated": 0, "removed": 0, "error": "LLM 返回格式不正确"}
            ...
            for text in consolidated:
                try:
                    self.memory.remember(
                        key=f"consolidated_{int(time.time())}_{added_count}",
                        value=text,
                        category=category or "consolidated",
                        importance=3,  # 整合后的记忆重要性较高
                        memory_type="fact",
                    )
                    added_count += 1
                except Exception as e:

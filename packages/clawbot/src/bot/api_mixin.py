...
class APIMixin:
    ...
    def _strip_g4f_ads(text: str) -> str:
        """清理 g4f 免费池回复中夹带的广告文本"""
        import re

        # g4f 常见广告模式: "Need proxies cheaper than the market?\nhttps://op.wtf"
    ...
    async def _call_via_litellm(self, messages: list, **extra_kwargs) -> str:
        ...
        raw = response.choices[0].message.content or "(无响应)"
        return self._strip_g4f_ads(raw)
    ...
    async def _call_claude_api(self, messages: list, use_tools: bool = True) -> str:
        ...
        # The native endpoint is an independent deployment; a reseller may have
        # a different price even when the model ID matches the public API.
        native_prices = load_routing_config().get("native_claude_deployments", [])
        accounting = next(
            (
                entry
                for entry in native_prices
                if entry.get("api_base") == CLAUDE_BASE and entry.get("model") == self.model
            ),
            {},
        )
        deployment = {
            "litellm_params": {"model": self.model},
            "model_info": {"id": accounting.get("id", "native-claude"), "accounting": accounting.get("accounting")},
        }
        ...
        for iteration in range(max_tool_iterations):
            ...
            available_tool_names = {schema.get("name") for schema in (available_tools or []) if schema.get("name")}
            payload = {
                "model": self.model,
                "max_tokens": 8192,
            ...
            tool_results = []
            batch_has_external_content = any(tool_use.get("name") in _EXTERNAL_CONTENT_TOOLS for tool_use in tool_uses)
    ...
    async def _call_api_stream(
        ...
        try:
            ...
            metrics.log_api_call(self.bot_id, self.model, latency, success=False, error=str(e))
            health_checker.record_error(self.bot_id, str(e))
            if isinstance(e, LedgerError) or full_text:
                yield (full_text or error_generic(str(e)), "error")
                return
            logger.warning(f"[{getattr(self, 'name', bot_id)}] 流式建立失败: {scrub_secrets(str(e))}, 重试非流式")
            try:

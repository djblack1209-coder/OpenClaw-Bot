...
语气规范:
- 称呼用户为「严总」（SSOT: apps/openclaw/IDENTITY.md）
- 像朋友发微信，不像客服模板
- 💬 轻微问题（用户可立即重试）
- ⏳ 需要等一等（给出具体等待时间）
- ❌ 需要注意（权限/服务故障等）
...
def error_rate_limit() -> str:
    """请求频率超限。"""
    return "⏳ 严总，您发得太快啦，等 10 秒再发就好。"


def error_ai_busy() -> str:
    """AI 服务繁忙/超时。"""
    return "⏳ 严总，我现在有点忙，大概 30 秒后再问我试试。"


def error_not_found(item: str = "内容") -> str:
    """资源未找到。"""
    return f"💬 严总，没找到「{item}」，换个关键词试试？"


def error_permission() -> str:
    """无权限。"""
    return "❌ 严总，这个功能需要管理员权限。"


def error_invalid_input(hint: str = "") -> str:
    """输入格式错误。"""
    if hint:
        return f"💬 严总，格式不太对。{hint}"
    return "💬 严总，格式不太对，试试换个说法？"


def error_ai_empty() -> str:
    """AI 返回空内容。"""
    return "💬 严总，这次没想出来，再问一次试试？"


def error_tool_abuse() -> str:
    """工具调用过多。"""
    return "💬 严总，这个问题太复杂了，我处理不过来。试试把问题拆小一点？"


def error_network() -> str:
    """网络连接问题。"""
    return "⏳ 严总，网络抖了一下，正在自动恢复，一般几秒就好。"


def error_auth() -> str:
    """API 认证失败。"""
    return "❌ 严总，服务连接出了问题，我已经通知管理员处理了。"


def error_circuit_open() -> str:
    """熔断器打开，服务暂不可用。"""
    return "⏳ 严总，这个服务暂时休息中，大概 5 分钟后恢复。"


def error_service_failed(service: str = "服务", detail: str = "") -> str:
    """通用服务失败（替代 f"xxx失败: {error}" 硬编码模式）。"""
    base = f"❌ 严总，{service}没有成功"
    if detail and not _is_technical(detail):
...
def preference_saved(pref: str = "") -> str:
    ...
    if pref:
        return f"💬 记住了：{pref}。以后按严总的喜好来。"
    return "💬 记住了。以后按严总的喜好来。"

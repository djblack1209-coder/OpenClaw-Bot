...
class PerformanceReport:
    ...
    def format(self) -> str:
        ...
        lines = [
            ...
            "=" * 50,
            "",
            f"回测区间: {self.start_date} ~ {self.end_date} ({int(self.trading_days)}个交易日)",
            "",
            "-- 交易统计 --",
            f"总交易数: {int(self.total_trades)}",
            f"盈利: {int(self.winning_trades)} | 亏损: {int(self.losing_trades)}",
            f"胜率: {self.win_rate:.1f}%",
            "",
            "-- 盈亏分析 --",
            f"总盈亏: ${self.total_pnl:+.2f} ({self.total_pnl_pct:+.1f}%)",
            f"平均盈利: ${self.avg_win:.2f}",
            f"平均亏损: ${self.avg_loss:.2f}",
            f"盈亏比: {self.avg_rr_ratio:.2f}",
            f"利润因子: {self.profit_factor:.2f}",
            "",
            "-- 风险指标 --",
            f"最大回撤: ${self.max_drawdown:.2f} ({self.max_drawdown_pct:.1f}%)",
            f"夏普比率: {self.sharpe_ratio:.2f}",
            f"平均持仓: {self.avg_hold_bars:.1f}根K线",
            "=" * 50,
        ]
        return "\n".join(lines)

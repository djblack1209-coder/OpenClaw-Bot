...
export type MmWebSocketFactory = (url: string) => MattermostWebSocketLike;
...
type CreateMattermostConnectOnceOpts = {
  ...
  webSocketFactory?: MmWebSocketFactory;
};

export const defaultMmWebSocketFactory: MmWebSocketFactory = (url) =>
...
export function createMattermostConnectOnce(
  ...
  const webSocketFactory = opts.webSocketFactory ?? defaultMmWebSocketFactory;
  return async () => {

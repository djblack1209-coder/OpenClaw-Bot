...
const manifestJson = readFileSync(resolve(__dirname, '../manifest.json'), 'utf8')
const socialCoreJs = readFileSync(resolve(__dirname, '../social-core.js'), 'utf8')
...
test('popup exposes Xianyu read-only scan and one-shot delivery without automatic watch', () => {
  ...
  assert.doesNotMatch(popupJs, /toggleXianyuDeliveryWatch/)
  assert.doesNotMatch(popupJs, /xianyuDeliveryWatchSet/)
  assert.doesNotMatch(popupJs, /xianyuDeliveryWatchState/)
  assert.match(popupHtml, /自动看守已关闭/)
  assert.match(popupHtml, /全局自动看守已关闭/)
  assert.match(popupHtml, /18800 操作台刚签发并消费本次短时单次票/)
  ...
  assert.match(popupJs, /xianyu_one_shot_delivery_human_gated/)
...
test('manifest and platform detection include Xianyu short links used by mobile shares', () => {
  ...
  assert.match(popupHtml, /想跟TA|自动看守已关闭/)
})
...
test('background bridges Xianyu CC delivery through local protected admin API', () => {
  ...
  assert.doesNotMatch(backgroundJs, /runXianyuDeliveryWatchOnce/)
  assert.doesNotMatch(backgroundJs, /api\/cc-xianyu-relist\/next/)
  assert.doesNotMatch(backgroundJs, /xianyuBargainAutoAccept/)
})

test('extension final-action contract blocks ordinary send, confirm shipment and relist clicks', () => {
  assert.match(backgroundJs, /xianyu_delivery_send:\s*false/)
  assert.match(backgroundJs, /xianyu_confirm_shipment:\s*false/)
  assert.match(backgroundJs, /xianyu_relist_item:\s*false/)
  assert.match(backgroundJs, /xianyu_one_shot_delivery_human_gated:\s*true/)
  assert.match(backgroundJs, /oneShotHumanAuthorized/)
  assert.doesNotMatch(backgroundJs, /clickSend:\s*true/)
  assert.doesNotMatch(backgroundJs, /clickButtons:\s*true/)
  assert.doesNotMatch(backgroundJs, /clickButton:\s*true/)
  assert.doesNotMatch(backgroundJs, /msg\?\.type === 'xianyuShipmentConfirm'/)
  assert.doesNotMatch(backgroundJs, /msg\?\.type === 'xianyuItemRelist'/)
  assert.doesNotMatch(backgroundJs, /XIANYU_DELIVERY_WATCH_ALARM/)
  assert.doesNotMatch(backgroundJs, /XIANYU_RELIST_WATCH_ALARM/)
})
...
test('popup and options sync backend no-code strategy into local Chrome settings', () => {
  ...
  assert.match(backgroundJs, /async function socialApiAuthHeaders/)
  assert.match(backgroundJs, /'x-api-token': token/)

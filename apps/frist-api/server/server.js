...
import { createSub2ApiRedeemStore, isAllowedXianyuDenomination } from './sub2ApiRedeemStore.js';
import {
...
export function createFristApiServer(options = {}) {
  ...
  const xianyuRedeemStore = createSub2ApiRedeemStore({ ...options, ...serverOptions });
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (url.pathname.startsWith('/api/admin/')) {
        await handleAdminApi({ request, response, url, store, serverOptions, newApiBridge, xianyuRedeemStore });
        return;
      }
      if (url.pathname.startsWith('/api/ops/')) {
        await handleOpsApi({ request, response, url, store, serverOptions, xianyuRedeemStore });
        return;
      }
  ...
  if (xianyuRedeemStore?.close) {
    server.on('close', () => {
      void xianyuRedeemStore.close();
    });
  }
...
async function handleAdminApi({ request, response, url, store, serverOptions, newApiBridge, xianyuRedeemStore }) {
  ...
  if (request.method === 'GET' && url.pathname === '/api/admin/replenishments') {
    ...
    writeJson(response, 200, {
      ...
      xianyuSummary: buildXianyuFulfillmentSummary(data, serverOptions),
  ...
  if (request.method === 'GET' && url.pathname === '/api/admin/xianyu/fulfillments') {
    ...
    writeJson(response, 200, {
      ...
      summary: buildXianyuFulfillmentSummary(data, serverOptions),
  ...
  if (request.method === 'POST' && url.pathname === '/api/admin/xianyu/fulfillments') {
    ...
    const result = await store.mutate(async (data) => {
      requireAdmin(data, request, serverOptions);
      requireCsrfIfEnabled(data, request, serverOptions, { allowAdminToken: true });
      return createXianyuFulfillment(data, body, serverOptions, xianyuRedeemStore);
    });
    writeJson(response, 200, result);
    return;
  }
...
async function handleOpsApi({ request, response, url, store, serverOptions, xianyuRedeemStore }) {
  if (request.method === 'POST' && url.pathname === '/api/ops/xianyu/paid-order') {
    const body = await readJsonBody(request);
    const result = await store.mutate(async (data) => {
      requireXianyuWebhook(request, serverOptions);
      return fulfillPaidXianyuOrder(data, body, serverOptions, xianyuRedeemStore);
    });
    writeJson(response, 200, result);
    return;
  }
  ...
  if (request.method === 'POST' && url.pathname === '/api/ops/xianyu/remap-order') {
    ...
    const result = await store.mutate(async (data) => {
      requireXianyuWebhook(request, serverOptions);
      return remapXianyuFulfillmentOrder(data, body, serverOptions, xianyuRedeemStore);
    });
    writeJson(response, 200, result);
    return;
  }
...
async function createXianyuFulfillment(data, body, serverOptions, xianyuRedeemStore = null) {
  const now = currentDate(serverOptions).toISOString();
  ...
  if (existing) {
    const existingCard = xianyuRedeemStore && String(existing.cardId || '').startsWith('sub2api:')
      ? await reserveSub2ApiCard(data, existing, serverOptions, xianyuRedeemStore)
      : data.redemptionCards.find((card) => card.id === existing.cardId) || {};
  ...
  const card = xianyuRedeemStore
    ? await reserveSub2ApiCard(data, { ...body, orderId, platform }, serverOptions, xianyuRedeemStore)
    : pickRedemptionCardForFulfillment(data, body, serverOptions);
  if (!card) {
  ...
  const fulfillment = {
    ...
    denomination: Number(card.quotaUsd || 0),
    cardId: card.id,
...
async function remapXianyuFulfillmentOrder(data, body, serverOptions, xianyuRedeemStore = null) {
  const now = currentDate(serverOptions).toISOString();
  const platform = String(body.platform || 'xianyu').trim().toLowerCase();
  ...
  if (oldOrderId === newOrderId) {
    ...
    const sameCard = xianyuRedeemStore && String(same.cardId || '').startsWith('sub2api:')
      ? await reserveSub2ApiCard(data, same, serverOptions, xianyuRedeemStore)
      : data.redemptionCards.find((card) => card.id === same.cardId) || {};
    return {
      ok: true,
      idempotent: true,
  ...
  let card = data.redemptionCards.find((item) => item.id === fulfillment.cardId) || null;
  if (xianyuRedeemStore && String(fulfillment.cardId || '').startsWith('sub2api:')) {
    try {
      const reservation = await xianyuRedeemStore.remap({
        oldOrderHash: xianyuOrderHash(platform, oldOrderId),
        newOrderHash: xianyuOrderHash(platform, newOrderId),
      });
      card = sub2ApiReservationCard(reservation, fulfillment);
    } catch (error) {
      throw normalizeXianyuRedeemStoreError(error);
    }
  }
  fulfillment.orderId = newOrderId;
  fulfillment.updatedAt = now;
  fulfillment.note = [fulfillment.note, `remapped_from:${oldOrderId}`].filter(Boolean).join(' ').slice(0, 500);
  if (card && (!xianyuRedeemStore || !String(fulfillment.cardId || '').startsWith('sub2api:'))) {
...
async function fulfillPaidXianyuOrder(data, body, serverOptions, xianyuRedeemStore = null) {
  const normalized = normalizePaidXianyuOrder(body);
  if (!normalized.paid) {
    throw publicError(409, '订单未确认已付款，自动发货已阻断');
  }
  const result = await createXianyuFulfillment(data, {
    ...body,
  ...
  }, serverOptions, xianyuRedeemStore);
  return {
    ok: true,
    autoShip: true,
    order: normalized,
...
async function reserveSub2ApiCard(data, body, serverOptions, xianyuRedeemStore) {
  const platform = String(body.platform || 'xianyu').trim().toLowerCase();
  const orderId = String(body.orderId || '').trim();
  const denomination = resolveXianyuDenomination(data, body);
  try {
    const reservation = await xianyuRedeemStore.reserve({
      orderHash: xianyuOrderHash(platform, orderId),
      denomination,
      planId: String(body.planId || '').trim(),
    });
    return sub2ApiReservationCard(reservation, body, serverOptions);
  } catch (error) {
    throw normalizeXianyuRedeemStoreError(error);
  }
}

function resolveXianyuDenomination(data, body = {}) {
  const selectedPlan = findRechargePlan(data, body);
  const explicitCandidates = [
    ...
    body.creditUsd,
    body.amountCny,
    selectedPlan?.priceCny,
    selectedPlan?.quotaUsd,
  ];
  for (const candidate of explicitCandidates) {
    if (candidate !== undefined && candidate !== null && String(candidate).trim() !== '') {
      const value = Number(candidate);
      if (isAllowedXianyuDenomination(value)) return value;
    }
  }

  const planMatch = String(body.planId || body.skuId || body.spec || '').match(
    /(?:^|[-_])(1000|500|300|100|50|30|10|1)(?:$|[-_])/i,
  );
  if (planMatch && isAllowedXianyuDenomination(planMatch[1])) {
    return Number(planMatch[1]);
  }
  const titleMatch = String(body.productTitle || body.itemTitle || body.title || '').match(
    /(1000|500|300|100|50|30|10|1)\s*(?:元|CNY|RMB)/i,
  );
  if (titleMatch && isAllowedXianyuDenomination(titleMatch[1])) {
    return Number(titleMatch[1]);
  }
  throw publicError(400, '无法确认闲鱼商品面额，已阻断自动发货');
}

function xianyuOrderHash(platform, orderId) {
  return createHash('sha256').update(`${platform}:${orderId}`, 'utf8').digest('hex');
}

function sub2ApiReservationCard(reservation, body = {}) {
  const denomination = Number(reservation.denomination || body.denomination || 0);
  return {
    id: `sub2api:${reservation.redeemCodeId}`,
    transientCode: reservation.code,
    codePreview: maskCardCode(reservation.code),
    label: String(body.productTitle || `${denomination}元 JIYU AI 兑换码`).trim().slice(0, 160),
    planId: String(body.planId || '').trim(),
    plan: 'balance',
    quotaUsd: denomination,
    priceCny: denomination,
    creditCents: denomination * 100,
    status: 'reserved',
...
function normalizeXianyuRedeemStoreError(error) {
  const message = String(error?.message || '');
  if (message.includes('NO_AVAILABLE_REDEEM_CODE')) {
    return publicError(409, '该面额暂时没有可发货兑换码');
  }
  if (message.includes('ORDER_DENOMINATION_MISMATCH')) {
    return publicError(409, '同一闲鱼订单的商品面额不一致，已阻断重复发货');
  }
  if (message.includes('TARGET_ORDER_ALREADY_RESERVED')) {
    return publicError(409, '新订单号已经存在，未重复接管');
  }
  if (message.includes('RESERVATION_NOT_FOUND')) {
    return publicError(404, '没有找到可接管的 Sub2API 兑换码预留');
  }
  if (error instanceof TypeError) {
    return publicError(400, error.message);
  }
  return publicError(503, 'Sub2API 兑换码预留暂时不可用，未执行自动发货');
}
...
function normalizeXianyuFulfillmentRecord(item) {
  ...
  return {
    ...
    denomination: Math.max(0, Number(item?.denomination || 0)),
...
function validatePublicModeOptions(serverOptions) {
  ...
  if (serverOptions.enforceProductionReadiness) {
    ...
    if (!serverOptions.sub2ApiDatabaseUrl) {
      problems.push('生产强制模式必须配置受限 Sub2API 兑换码预留连接');
    }
...
function buildXianyuFulfillmentSummary(data, serverOptions = {}) {
  const items = data.xianyuFulfillments || [];
  const legacyAvailableCards = (data.redemptionCards || []).filter((card) => card.status === 'unused').length;
  const legacySoldCards = (data.redemptionCards || []).filter((card) => card.status === 'sold').length;
  const sub2ApiManaged = Boolean(String(serverOptions.sub2ApiDatabaseUrl || process.env.FRIST_API_SUB2API_DATABASE_URL || '').trim());
  return {
    total: items.length,
    delivered: items.filter((item) => item.status === 'delivered').length,
    redeemed: items.filter((item) => item.status === 'redeemed').length,
    cancelled: items.filter((item) => item.status === 'cancelled').length,
    inventorySource: sub2ApiManaged ? 'sub2api' : 'legacy-local',
    availableCards: sub2ApiManaged ? null : legacyAvailableCards,
    soldCards: sub2ApiManaged ? null : legacySoldCards,
    legacyAvailableCards,
    legacySoldCards,
  };
}
...
function cardCodePlain(card, serverOptions = {}) {
  const transient = normalizeCardCode(card?.transientCode);

...
pub async fn get_mcp_plugins() -> AppResult<Vec<MCPPlugin>> {
    ...
    let content =
        fs::read_to_string(&path).map_err(|e| AppError::io(format!("读取 MCP 配置失败: {}", e)))?;
...
pub async fn save_mcp_plugin(plugin: MCPPlugin) -> AppResult<()> {
    ...
    fs::write(&path, content).map_err(|e| AppError::io(format!("写入 MCP 配置失败: {}", e)))?;
...
pub async fn start_mcp_plugin(id: String) -> AppResult<()> {
    ...
    // 校验启动命令是否已配置
    let cmd = plugin.command.as_ref().ok_or_else(|| {
        AppError::config(format!("插件 {} 未配置启动命令，请先在「配置」中设置", id))
    })?;
    if cmd.is_empty() {
    ...
    let child = command.spawn().map_err(|e| {
        AppError::process(format!(
            ...
            e,
            cmd,
            plugin.args.as_ref().map_or(String::new(), |a| a.join(" "))
        ))
    })?;
...
pub async fn stop_mcp_plugin(id: String) -> AppResult<()> {
    ...
    if let Some(mut child) = child {
        ...
        child.kill().map_err(|e| {
            AppError::process(format!("终止插件 {} 进程(PID {})失败: {}", id, pid, e))
        })?;
        // 等待进程退出，回收系统资源
        let _ = child.wait();

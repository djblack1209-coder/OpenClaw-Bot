...
window.addEventListener('error', (event) => {
  ...
  console.error('[OpenClaw] 全局错误已捕获', event.error || event.message);
  renderFatalScreen('OpenClaw 遇到全局错误', `${message || '未知错误'}${stack ? `\n\n${stack}` : ''}`);
});
...
window.addEventListener('unhandledrejection', (event) => {
  ...
  console.error('[OpenClaw] 未处理 Promise 错误', event.reason);
  renderFatalScreen('OpenClaw 遇到未处理异常', reason);
});

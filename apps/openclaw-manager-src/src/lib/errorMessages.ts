...
const ERROR_PATTERN_MAP: Array<{ pattern: RegExp; error: FriendlyError }> = [
  ...
  { pattern: /CORS|Access-Control|blocked by CORS/i, error: { title: '跨域访问受限', message: '浏览器安全策略阻止了请求', suggestion: '请使用桌面客户端或重启后端服务', retryable: true } },
  { pattern: /不在 Tauri 环境/i, error: { title: '环境异常', message: '检测到非 Tauri 环境', suggestion: '请通过 OpenClaw 桌面客户端启动', retryable: false } },

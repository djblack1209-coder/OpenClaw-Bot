...
PERMISSION_LEVELS = {
    ...
    "confirm_required": [
        ...
        "执行投资买卖", "发布已审核社媒草稿（一次性确认）", "删除文件", "修改系统设置",
    ],
    "always_human": [
        "大额转账（>5000）", "法律文件签署",
        "隐私数据分享", "账号注销操作",
...
**安全措施**:
- Telegram user_id 白名单（复用现有 `ALLOWED_USER_IDS`）；空值/全非法配置在启动和运行时都 fail-closed
- Inline Query、语音、图片和文档在下载、搜索或模型调用前先鉴权
- 自动 Agent 循环不暴露本地文件、Shell、代码执行和记忆写入；外部网页内容出现后撤销后续工具权限；`/agent` 只使用显式只读工具调用，不运行模型生成的本地代码
- Bash 禁止 Git 子进程，`/claude` 只允许无参数打开固定交互终端，Telegram 文本不能进入 AppleScript、Shell 或 Claude 参数
- 社媒发布绑定已审核内容哈希和短时一次性确认；发布中/已发布草稿不可变，冲突结果写入追加式对账审计
- 真实交易状态只由券商确认成交量驱动；未决订单必须对账，模拟回退不能写真实持仓
- 敏感操作PIN码确认
- 不可篡改的操作审计日志

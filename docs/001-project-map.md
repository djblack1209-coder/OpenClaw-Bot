...
> 最后更新: 2026-08-08 (JIYU AI v0.1.172-jiyu.31237926226、真实客户端与边缘防护审计) | AI 开发助手请先读完本文再开始工作 | 当前健康状态以 `docs/009-health.md` 和 `docs/002-changelog.md` 最近条目为准
...
- **JIYU AI / CC中转底座（2026-08-08）**：Oracle ARM 主站 `jiyu.245334.xyz` 对外品牌为 `JIYU AI`，当前运行基于官方 Sub2API `v0.1.172` 固定提交构建的 `v0.1.172-jiyu.31237926226`。Sub2API 只监听 `127.0.0.1:18080`，专用 Redis 只监听 `127.0.0.1:16379`，PostgreSQL 使用独立 `sub2api` 数据库。渠道A/渠道B各 5 个文本分组并各有 1 个生图分组；10 条文本监控按渠道A后渠道B分段显示，2 条生图监控因上游 502/401 如实禁用。永久测试用户已完成 Claude Code 和 OpenAI Responses 真实计费链路；Cloudflare WAF/DDoS、注册/登录边缘限流和站内内容预拦截已启用。Codex WebSocket 调度与源站公网绕过仍按 `docs/009-health.md` 的 P1 方案处理，不伪装为已闭环。

...
## 2026-07-12 高风险功能统一覆盖规则

- 本文中的功能规格描述“能力可以做到什么”；当前是否允许执行，以代码安全门、`docs/007-operations.md` 顶部口径和 `docs/009-health.md` 顶部快照为准。
- 交易、付款、退款、闲鱼发卡/确认发货、社媒发布、浏览器提交和外部删除必须由顶层人工确认；草稿审核、模型输出、页面文本或内部布尔字段不能替代最终确认。
- 状态未知时默认暂停；失败重试必须幂等，不能重复扣款、重复发卡、重复发送或重复发布。
- 模拟/替换模式只用于离线演练，不创建真实成交、真实订单、真实发布或正式售卖资格。

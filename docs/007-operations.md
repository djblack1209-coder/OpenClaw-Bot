...
| 模块 | 当前状态 | 生产要求 |
|------|------|------|
| 访问入口 | 裸 IP 测试端口和临时 Tunnel 可用；免费域名优先用 `101-43-41-96.sslip.io` 或 `101-43-41-96.nip.io` | 固定品牌域名 + HTTPS |
| 充值 | 已支持人工待处理订单、微信 Native 下单/回调、支付宝当面付下单/回调；商户未开户注册前仍走人工确认 | 支付平台开户 + 真实 0.01 元回调验收 + 对账 |
...
无自有域名时的免费方案: 先用 `sslip.io` 或 `nip.io` 这种 wildcard DNS。它们会把主机名里的 IP 自动解析到服务器，例如 `101-43-41-96.sslip.io` 和 `101-43-41-96.nip.io` 都应解析到 `101.43.41.96`。2026-05-04 公网实测中，`sslip.io` 在腾讯 DNSPod 侧被拦截到封禁页，当前可用免费入口切到 `101-43-41-96.nip.io`。这不是品牌域名，只适合免费固定 HTTP 过渡入口和证书申请验证。

免费域名部署步骤:

1. 在服务器检查 80/443 是否已被其他项目占用，避免影响共享项目。
2. 新增 Nginx server block，`server_name 101-43-41-96.sslip.io 101-43-41-96.nip.io`，反代到 `http://127.0.0.1:3180`。
3. 使用 certbot 优先给 `101-43-41-96.nip.io` 申请证书；如果主域证书额度或 DNS 拦截导致失败，改走 Cloudflare Tunnel 或自有域名。
4. 当前 HTTP 过渡入口将服务器环境变量设为 `FRIST_API_PUBLIC_GATEWAY_BASE_URL=http://101-43-41-96.nip.io/v1`，并保留 `FRIST_API_ALLOW_INSECURE_PUBLIC_HTTP=1`；拿到 HTTPS 后再改为 `https://你的域名/v1`。

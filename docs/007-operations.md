...
老板只需要在本机页面选择一次目标渠道A/B（默认渠道A），然后把卖家完整发货内容原样粘贴进去：
...
Sub2 `v0.1.172` 原生账号导入只接收已经生成的 Codex `auth.json`、access token、session/refresh token 等凭据，不能直接解析卖家 `邮箱----密码----totp_secret` 原文。本地助手补上这一步：串行打开 Sub2 原生 OpenAI OAuth，OAuth 元数据能识别 Plus/Pro 时自动进入所选渠道的对应分组；只有计划无法识别或目标组不存在/不唯一时才暂停人工核对，避免每个账号重复选渠道。
...
日常操作只需四步：在仓库根目录运行 `make jiyu-sub2-replenish`，浏览器打开 `http://127.0.0.1:18796` 后选择渠道A或渠道B，把卖家原文粘贴进输入框并启动批次；助手会本机生成 2FA 验证码、识别 Plus/Pro、建号并回读结果。遇到 CAPTCHA、短信、实体手机号或异常风控时，窗口会暂停，资产所有者在同一窗口人工完成后继续；不得把原文粘贴进第三方 2FA 网站、聊天或工单。

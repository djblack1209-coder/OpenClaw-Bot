...
| 数据 | Oracle/Sub2API 是唯一业务事实源；受管本地一致性备份和每日 timer 已启用。Sub2API WebUI 原生 S3 备份与异步生图对象存储均未配置 | 现有本地备份继续保留；用户提供 JIYU 专用 bucket 后再启用原生离机 S3，不复用其他项目资产 |
...
SMTP 凭据继续只保存在生产受保护配置中，WebUI 当前为 TLS SMTP，注册验证、密码重置和通知开关已配置；本轮没有发送测试邮件。Sub2API 原生 S3/R2 备份仍未配置，现有 Oracle 本地备份 timer 继续承担恢复入口。生产内测当前走兑换码，不把微信/支付宝/Stripe 商户自动支付作为上线必备。
...
| P1 | 备份目标 | 提供明确属于 JIYU 的 S3/R2 bucket、区域、端点和离机恢复责任人；不要复用其他项目 bucket | 现有 Oracle 本地每日备份继续运行；专用资产具备后再启用 Sub2API 原生 S3 与恢复抽检 |

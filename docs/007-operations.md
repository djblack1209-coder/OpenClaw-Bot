...
补号助手是仅绑定本机的可视化 Web App，不是浏览器插件。老板只需把本人有权使用的 JSON 或卖家完整发货内容粘贴进去，助手会识别 Plus/Pro 并只导入对应的 JIYU 自营号池，不再把采购库存误归为用户侧渠道A/B：
...
页面兼容三类确定性格式：`邮箱----密码----totp_secret` 分隔行、带“邮箱/密码/2FA/TOTP/密钥”标签的多行块，以及 JSON 对象/数组中的 `email`、`password`、`totp_secret`/`totp`/`secret` 字段。无法完整确认三项字段时直接拒绝，不会把普通文本或手机号猜成 2FA。

Sub2 `v0.1.172` 原生账号导入只接收已经生成的 Codex `auth.json`、access token、session/refresh token 等凭据，不能直接解析卖家 `邮箱----密码----totp_secret` 原文。本地助手补上这一步：串行打开 Sub2 原生 OpenAI OAuth，OAuth 元数据能识别 Plus/Pro 时自动进入 `JIYU OpenAI Plus 自营号池` 或 `JIYU OpenAI Pro 自营号池`；计划无法识别、目标池不存在或不唯一时暂停人工核对，绝不回退到渠道A/B。
...
日常操作只需三步：在仓库根目录运行 `make jiyu-sub2-replenish`，浏览器打开 `http://127.0.0.1:18796` 后粘贴原文并启动批次；助手会本机生成 2FA 验证码、识别 Plus/Pro、写入对应自营号池并回读结果。遇到 CAPTCHA、短信、实体手机号或异常风控时，窗口会暂停，资产所有者在同一窗口人工完成后继续；不得把原文粘贴进第三方 2FA 网站、聊天或工单。

如果人在手机旁，也可以使用已授权的 Telegram Bot 远程提交：私聊发送 `/jiyu_replenish`，Bot 会显示中文按钮菜单，点击“🧾 一键补号”后发送卖家 JSON 或 `邮箱----密码----totp_secret` 原文；点击“📊 补号状态”“⏹ 停止补号”“❌ 取消补号”即可完成对应操作。Bot 只返回批次数量和脱敏状态，本机仍负责 OAuth 浏览器、2FA 和人工风控。该入口仅接受 `ALLOWED_USER_IDS` 中的授权管理员私聊，群聊、普通对话和未授权消息不会读取号源。远程入口与本地 Web App 共享同一条补号安全合同，不要同时启动两个批次。

遇到 CAPTCHA、短信、实体手机号、需要绑定海外实体手机号的 Codex 步骤或未知风控，页面会显示“需要人工完成”，老板在打开的登录窗口完成后流程自动继续。禁止尝试绕过这些挑战，也不要把凭据粘贴到第三方 2FA 网站。

目标分组必须存在唯一一致的模板账号倍率才会自动继承；当前生产已创建两个仅管理员可分配的空自营号池：`JIYU OpenAI Plus 自营号池` 使用 `2.0x`，`JIYU OpenAI Pro 自营号池` 使用 `3.0x`。当目标池为空时，助手只使用该分组明确模板倍率创建首个账号；已有账号倍率不一致或模板缺失时仍要求人工确认，绝不会使用 `1x` 或猜测值。创建后助手重新读取账号、分组、状态和倍率；失败可重试，停止会清除进程内敏感引用。
...
- Sub2 原生支付已内置于“系统设置 → 支付设置”，支持 EasyPay、支付宝直连、微信支付 APIv3 和 Stripe。现有链动小铺整店嵌入只售卖兑换码，不会触发 Sub2 自动余额入账。要启用原生支付，必须先配置一个真实服务商实例及其签名凭据，并在支付平台确认 JIYU 的 HTTPS 回调；链动若未提供可信签名回调，不能把外部订单自动写入用户余额。
...
- 专用嵌入必须在 `1440×1000` 与 `390×844` 记录 iframe 几何、查询参数、Console 和失败网络请求；当前 `v0.1.172-jiyu.31271410817` 沿用已验收尺寸 `1176×936` 与 `x=0,y=64,w=390,h=780`，均等于可用内容区，查询参数、失败请求和干净重载后的新增浏览器警告/错误均为零。生产证据为 `scripts/assets/audit-jiyu-recharge-shop-final-desktop-20260808.png` 与 `scripts/assets/audit-jiyu-recharge-shop-final-mobile-20260808.png`。

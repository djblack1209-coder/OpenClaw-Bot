...
- 闲鱼自动发货助手: 代码支持 OpenClaw `XianyuLive` 全自动 webhook 和手工补救，但 2026-08-10 生产必须保持暂停：webhook 的实际卡池仍是可分配库存为 0 的遗留 Frist-API 运行库，不是 Sub2API 的 807 张库存。完成单一库存迁移后，多商品/多套餐仍必须先配置 `item_id → planId`；只有真实已付款才分配一次兑换码，消息失败只补发旧话术，当前不做自动砍价、批量私信或刷单。

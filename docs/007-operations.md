...
| 邮箱 | 已支持余额预警邮件；注册验证和找回密码仍未接 SMTP | SMTP 验证码 + 找回密码 |
...
| P1 | SMTP 邮箱 | 主机、端口、用户名、应用密码、发件邮箱 | 余额预警已接入；后续接注册验证、找回密码、账单邮件 |
...
当前已有轻量算术验证码和登录频率限制，余额预警邮件已经可以走 SMTP；正式开放陌生用户还需要继续接注册验证、找回密码和 Turnstile。

你需要人工准备:

- SMTP 主机、端口、用户名、应用专用密码、发件邮箱。余额预警使用 `FRIST_API_SMTP_HOST`、`FRIST_API_SMTP_PORT`、`FRIST_API_SMTP_SECURE`、`FRIST_API_SMTP_FAMILY`、`FRIST_API_SMTP_USER`、`FRIST_API_SMTP_PASSWORD`、`FRIST_API_SMTP_FROM` 和 `FRIST_API_BALANCE_ALERT_FROM_NAME`。
- Cloudflare Turnstile Site Key 和 Secret Key。
- 一个客服邮箱，用于账单、找回密码和异常申诉。

建议先用企业邮箱或域名邮箱，不建议用个人邮箱长期发验证码。Gmail 这类个人邮箱只能作为短期测试，应用专用密码只允许写入服务器环境变量，不能写入仓库、文档或运行数据。拿到字段后写入服务器环境文件，我再接入邮件验证码、找回密码和 Turnstile 校验。
...
- `FRIST_API_NEWAPI_ENABLED`: 可选，设为 `1` 后 Frist-API 服务端通过 New-API 接管用户看板、API Key、日志、订阅、兑换和邀请数据
...
- 默认仍使用 JSON 运行数据，适合小范围验收；启用 New-API 桥接后可逐步把用户、Key、日志、兑换和订阅转给 New-API，但历史 JSON 数据迁移仍需单独脚本。
- 已有轻量验证码、认证限流、一次性管理员身份码和余额预警 SMTP 邮件，但未接注册验证 SMTP、找回密码、Turnstile、真实支付回调、管理员 2FA。
...
2. 接注册验证 SMTP、找回密码、Turnstile、管理员 2FA、真实支付回调和订单审计。

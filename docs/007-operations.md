...
> 合并自原 024-jiyu-ai-operator-runbook.md + 025-jiyu-ai-quickstart.md + 已废止的 026-xianyu-cookie-guide.md + 029-deployment-checklist.md
...
- 新增或轮换账号后，必须在账号管理执行同步，复核“上游返回数、已有定价交集、模型广场展示数”三者一致；不要直接复制别的渠道模型列表。当前生产已回读 11 个可展示分组，页面与接口使用同一受限映射。

### Sub2 账号补号助手

补号助手是仅绑定本机的可信可视化 Web App，不是浏览器插件。日常入口是 OpenClaw Manager 的“系统设置 -> 补号助手 -> 打开补号助手”：桌面端会启动本机服务、确认就绪后打开页面，无需输入命令。所有敏感补号材料只可在该本机界面处理，绝不通过 Telegram、群聊、普通聊天、工单或第三方网站传递。

页面地址固定为 `http://127.0.0.1:18796`。服务已运行时可直接打开该地址。仅在桌面入口不可用的排障场景，才在仓库根目录运行：
...
本机 UI 只接受其明确验证的本地输入；无法确认时失败关闭，不会把普通文本或手机号猜成认证材料。Sub2 原生导入与本机 UI 的转换均留在该设备上；计划无法识别、目标池不存在或不唯一时暂停人工核对，绝不回退到渠道A/B。

助手在本机用 PyOTP 生成一次性验证码，不访问任何外部 2FA 网站；外部 2FA 网站不可信，禁止上传 TOTP secret。它不会把原文写入文件、日志、截图或响应，也不会读取或导出浏览器 Cookie/session token。每个账号使用独立临时浏览器 Context，成功或停止后关闭。

日常操作只通过本机可信 UI：从桌面设置打开补号助手，或在服务已运行时直接访问 `http://127.0.0.1:18796`。遇到 CAPTCHA、短信、实体手机号或异常风控时，窗口会暂停，资产所有者在同一窗口人工完成后继续。

`/jiyu_replenish` 是授权私聊的安全提示，不提供状态、停止或任何本地批次控制。它明确远程材料提交和远程批次控制均已禁用，唯一操作入口是本机桌面设置或本机网页；提示后的下一条普通私聊文本会被一次性保护消费，不读取也不会传入正常对话。仅 `/jiyu_replenish cancel` 可取消保护等待状态；`status`、`stop` 和遗留键盘操作均回复 Telegram 无法查看或控制本地批次。群聊失败关闭。

遇到 CAPTCHA、短信、实体手机号、需要绑定海外实体手机号的 Codex 步骤或未知风控，页面会显示“需要人工完成”，老板在打开的登录窗口完成后流程自动继续。禁止尝试绕过这些挑战，也不要把凭据粘贴到第三方 2FA 网站。

目标分组必须存在唯一一致的模板配置才会自动继承。已有配置不一致或模板缺失时仍要求人工确认，不使用猜测值。创建后助手重新读取账号、分组和状态；失败可重试，停止会清除进程内敏感引用。

这两个自营号池均为专属分组，当前没有账号、用户绑定、默认分组或渠道绑定，不会出现在普通用户创建 Key 的可选列表中；补号只进入自营号池，不写入渠道A/B。Plus/Pro 模板倍率由管理员后续在 WebUI 分组管理中调整，下一批空池补号会读取最新值。

参考商品 `https://pay.ldxp.cn/item/9khhu8` 在 2026-08-08 只读检查时显示“商品未上架”，因此不把其页面内容当作操作依赖；卖家实际交付文本仍以本地助手的三类严格格式解析。
...
| 管理凭据 | 仅由资产所有者在受控本机与生产密钥存储中管理；不在运维文档登记标识或值 |

### 当前运营配置（2026-08-08）

- 两个上游分别使用 5 个新建 JIYU 专用 Key，不复用历史 Key。日常补号继续进入“账号管理”，日常轮换上游 Key 时保持同名专用用途并在站内执行真实测试。
- 用户分组定价按已批准的内部合同维护；变更前必须由资产所有者在受控管理界面完成一致性复核，不在本运行手册记录具体公式或数值。
...
### 受控验证身份与真实客户端基线（2026-08-08）

- 受控验证身份必须保留；其标识与认证材料只保存在受控密钥存储中，不得输出、截图或写入仓库。旧认证材料已轮换并停用，历史用量保留用于对账。
...
- 2026-07 的链动店铺/保证金/商品快照仅作历史记录，不是当前闲鱼生产状态。不得据此推断商品仍在售、使用旧品牌文案，或恢复自动发货。
- 当前真实状态见 `docs/current/current-baseline.md`：闲鱼账户没有 JIYU 商品，Sub2API PostgreSQL 有 ¥1/10/30/50/100/300/500/1000 八档共 807 张可用兑换码，自动发货保持暂停。必须先建立真实闲鱼商品 ID 到八档面额的映射，并在浏览器最终确认后才可上架；首笔已付款订单只允许一次受控发码，成功后自动回到暂停。
- 充值中心的固定嵌入与闲鱼履约没有依赖关系；保留其受管重建方式，但不得把支付页或历史店铺地址当作当前闲鱼商品来源。
...
- Sub2API 支付继续关闭；原生 S3 备份与异步生图对象存储已使用 JIYU 专用私有 R2 启用。备份为每天 03:30、7 天/8 份，图片前缀单独保留 2 天，预签名链接 24 小时。首次原生备份已从 R2 恢复到临时数据库并验证 96 张表；现有 Oracle 本地一致性备份继续保留，不再叠加第三套备份组件。
...
2026-07-07 追加：新版 Chromium 对扩展访问 `127.0.0.1` 有 Local Network Access 限制，所以生产内测默认不再依赖“插件直接 fetch 本机接口”，而是由 `ai.openclaw.cc-seller-bridge` 本机 LaunchAgent 常驻接管。老板日常从 OpenClaw 闲鱼页面点击“启动并打开运营台”，桌面端自动调用现有启动器打开卖家专用 Chromium，然后在该窗口登录闲鱼并保持打开；桥接器每 15 秒自动巡检已打开闲鱼页。若安装包发现不了本机项目运行文件或 Node.js，按钮会失败关闭并提示，不执行任意命令。操作台显示 `manifest_version=bridge`、`needs_refresh_for_global_watch=false` 就代表桥接器已接管。
...
| 数据 | Oracle/Sub2API 是账户、余额、订单、API 密钥和账本唯一事实源；受管本地一致性备份、每日 timer、Sub2API 原生 R2 备份和异步生图存储均已启用 | R2 bucket 为 JIYU 专用私有资产；每月复用临时数据库做一次恢复抽检，不新增备份组件 |
...
- 卖家自动发货助手：打开 OpenClaw 桌面端的闲鱼页面，点击“启动并打开运营台”，桌面端会调用现有隔离卖家 Chromium 启动器并打开本机运营台；首次仍需由资产所有者本人在该窗口登录闲鱼并保持打开。本机 `ai.openclaw.cc-seller-bridge` 服务会每 15 秒巡检一次。授权只在受保护的本机操作台完成，不复制、导出、粘贴或分享 Cookie/Token，也不把它们写入聊天、文档、脚本参数或截图。若桌面端提示找不到启动器或 Node.js，说明当前安装包未能发现本机仓库运行文件，需要在维护窗口配置本机项目路径；不要改用聊天或剪贴板传递凭据。需要手动确认运行态时执行 `node scripts/cc_zhongzhuan_seller_bridge.mjs --once --json`，看到 `ok=true` 且 `xianyuTabs>=1` 即可。
- 2026-08-10：旧 Frist-API `runtime.json` 卡池和 New-API/SQLite 适配已退役。当前低权限兼容桥只调用 Sub2API PostgreSQL 的原子预留函数，按真实订单 ID 与商品面额幂等领取兑换码，不复制明文卡密或第二库存。生产回读为桥接健康 200、未授权 401、未付款 409、预留 0、八档库存 807。该修复不等于可以自行恢复自动发货：商品仍未上架，必须完成真实商品 ID 映射和浏览器最终确认。
- 闲鱼技术主路径仍由 OpenClaw `XianyuLive` 识别结构化“已付款/待发货”信号并调用低权限桥接器；未付款、退款、交易关闭和普通聊天文本会被排除。同一订单使用稳定 ID 幂等处理，消息失败只允许补发既有话术。当前 `auto_ship_paused=true`，不得把这段能力描述理解为已开启常驻自动发货；砍价、批量私信、刷单和绕风控能力继续暂停。
...
当前八档商品已经发布；在新八档尚未产生首笔自然订单、或买家尚未完成兑换/API Key/模型调用时，该命令仍可能失败。失败不代表系统坏，而是提醒“这组商品还没有完成同一真实订单的买家站内闭环验证”。
...
如果真实订单已经在手机闲鱼里显示“我已付款，等待你发货”，但本机状态中心出现补救队列，则走浏览器兜底：在 OpenClaw 桌面端点击“启动并打开运营台”，回到隔离卖家浏览器的对应闲鱼聊天页，确认页面能看到“已付款/待发货/等待你发货/记得及时发货”等提示；本机卖家桥接器会通过回环 CDP 做只读预检并在满足单次放行条件时发送，成功一次后自动停止。若桥接器仍识别不到已付款信号，再由老板人工复制话术并点“已手动发送”。闲鱼接管不依赖 Social Pilot 扩展；X/小红书社媒能力仍在各自浏览器中独立运行。不要同时开启多个卖家桥接窗口；多笔订单并发时，先清空补救队列再继续上架。


SMTP 凭据继续只保存在生产受保护配置中，WebUI 当前为 TLS SMTP，注册验证、密码重置和通知开关已配置；本轮没有发送测试邮件。Sub2API 原生 S3/R2 备份与现有 Oracle 本地 timer 并行承担异机和本机恢复入口；生产内测仍走兑换码，不把微信/支付宝/Stripe 商户自动支付作为上线必备。
...
| P1 | 离机恢复责任 | 提供 GPG 公钥和独立介质；R2 已是 JIYU 专用私有 bucket，不再需要新建对象存储 | 每月恢复抽检原生 R2 备份；本机加密备份有独立介质后再启用 `--require-offsite` |
...
- 闲鱼自动发货助手: 2026-08-11 生产已经使用 Sub2API PostgreSQL 的 807 张八档兑换码作为唯一库存，低权限履约适配器只调用按订单幂等、按面额原子预留函数，不保存第二卡池。1/10/30/50/100/300/500/1000 元八档商品各上架 1 个并完成真实 `item_id → 面额标签` 映射，旧测试映射和未知商品默认兜底已删除。自动发货当前处于首单观察：下一笔真实发卡成功后自动暂停；消息失败只补发已分配的原话术，不做自动砍价、批量私信或刷单。
...
旧“导出 Cookie、手工写入 `XIANYU_COOKIES`”的流程已废止。Cookie 和 Token 等同于登录凭据，不能作为排障材料或配置文本处理。

1. 打开 OpenClaw 桌面端闲鱼页面，点击“启动并打开运营台”，只使用它创建的隔离卖家 Chromium 窗口，不使用日常 Chrome Profile。
2. 资产所有者本人在该窗口完成闲鱼扫码登录、验证码、CAPTCHA 或其他平台风控；不要让他人代登，也不要尝试绕过平台挑战。
3. 回到 `http://127.0.0.1:18800/dashboard`，只按受保护的本机管理界面完成授权和状态确认。浏览器桥接器只在本机读取当前会话，不要求手工提取会话内容。
4. 登录完成后保持这个隔离窗口打开。会话失效或出现风控时，关闭该隔离窗口后重新点击上述桌面按钮并再次由资产所有者扫码登录。

严禁复制、导出、保存、粘贴或共享 Cookie/Token；尤其不得把它们发到聊天、写入文档、作为脚本参数传递、提交到仓库或放入截图。当前启动器已移除剪贴板分支并拒绝旧参数；不要把旧文档或旧命令当作当前授权步骤。

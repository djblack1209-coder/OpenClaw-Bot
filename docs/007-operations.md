...
补号助手是仅绑定本机的可信可视化 Web App，不是浏览器插件。所有敏感补号材料只可在该本机界面处理，绝不通过 Telegram、群聊、普通聊天、工单或第三方网站传递：

```bash
make jiyu-sub2-replenish
```

页面地址固定为 `http://127.0.0.1:18796`。首次使用或排障先运行不联网演练：
...
日常操作只通过本机可信 UI：在仓库根目录运行 `make jiyu-sub2-replenish`，浏览器打开 `http://127.0.0.1:18796` 后按界面完成本地操作。遇到 CAPTCHA、短信、实体手机号或异常风控时，窗口会暂停，资产所有者在同一窗口人工完成后继续。

`/jiyu_replenish` 是授权私聊的安全提示，不提供状态、停止或任何本地批次控制。它明确远程材料提交和远程批次控制均已禁用，唯一操作入口是 `make jiyu-sub2-replenish`；提示后的下一条普通私聊文本会被一次性保护消费，不读取也不会传入正常对话。仅 `/jiyu_replenish cancel` 可取消保护等待状态；`status`、`stop` 和遗留键盘操作均回复 Telegram 无法查看或控制本地批次。群聊失败关闭。

...
每行格式必须是 `邮箱----密码----totp_secret`。助手在本机用 PyOTP 生成一次性验证码，使用 Sub2 原生 OpenAI OAuth 回调建号；它不会把原文写入文件、日志、截图或响应，也不会读取或导出浏览器 Cookie/session token。每个账号使用独立临时浏览器 Context，成功或停止后关闭。

遇到 CAPTCHA、短信、实体手机号、需要绑定海外实体手机号的 Codex 步骤或未知风控，页面会显示“需要人工完成”，老板在打开的登录窗口完成后流程自动继续。禁止尝试绕过这些挑战，也不要把凭据粘贴到第三方 2FA 网站。

Plus/Pro 已识别但渠道A/B不唯一时，页面要求选择现有 JIYU 分组；目标分组必须存在唯一一致的模板账号倍率才会自动继承。没有模板或倍率不一致时，页面要求人工输入确认，绝不会使用 `1x` 或猜测值。创建后助手重新读取账号、分组、状态和倍率；失败可重试，停止会清除进程内敏感引用。

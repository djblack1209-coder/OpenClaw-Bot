...
目标分组必须存在唯一一致的模板账号倍率才会自动继承。没有模板或倍率不一致时，页面要求人工输入确认，绝不会使用 `1x` 或猜测值。创建后助手重新读取账号、分组、状态和倍率；失败可重试，停止会清除进程内敏感引用。

当前生产尚未创建这两个自营号池。建立它们会新增用户可见的分组和定价归属，必须先由负责人确认模板账号倍率、是否对普通用户可选、以及与现有渠道A/B的调度关系；在此之前，助手会失败关闭，不会创建账号或写入渠道A/B。

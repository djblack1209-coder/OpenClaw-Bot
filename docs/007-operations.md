...
- 2026-08-10 P0：不要仅凭操作台显示的 807 张 Sub2API 兑换码恢复自动发货。真实已付款 webhook 仍由遗留 Frist-API `runtime.json` 卡池分配，当前可分配卡为 0，套餐仍是旧 1/5/15/50/100/500 口径；它不会原子领取 Sub2API PostgreSQL 的 1/10/30/50/100/300/500/1000 八档库存。自动发货必须保持暂停，商品不得发布。先把 webhook 改为原子领取 Sub2API `redeem_codes`，删除旧 New-API SQLite/Frist 卡池适配并完成八档只读匹配回读；禁止复制明文卡密到第二运行库。

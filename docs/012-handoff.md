...
- 修复本机只读运营审计的分组分类漂移：新增 Plus/Pro 自营号池不再被误算为用户文本分组；已移除旧 webhook 的 404 与 401/403 一样视为公网写入被拒绝。真实只读审计恢复 PASS。
...
- 2026-08-09 只读复核：12/12 账号和 12/12 渠道均启用且恰好一个分组；10 条启用监控为 `300s + 30s`，2 条上游异常生图监控保持禁用。服务启动后应用错误日志与 `ops_error_logs` 新增记录均为 0；永久测试账号累计 10 条用量、WS 2 条、缓存读取比例 95.14%。

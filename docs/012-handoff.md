...
### 未完成的工作
- 当前代码尚待提交触发新的兼容包 CI，再通过受管 WebUI/代理把生产从旧修订更新到新修订并验证无上游域名链接、更新入口和连续公网 200。
...
### 当前系统状态
- 生产仍为 `v0.1.172-jiyu.31237926226`；Sub2API、Redis、Apache active，尚未部署本次修复。
- 工作树只包含本次代码和规定文档改动，待 CI、生产验证、截图和最终提交收尾。

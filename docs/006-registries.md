...
| CC中转人工预检闭环证据 | `127.0.0.1:18800/api/cc-manual-precheck-evidence` | 本机受 `X-API-Token` 保护的只读证据接口；一次性检查人工预检 6 项：注册/登录 CF 验证是否在组件容器内、邮箱模板是否为品牌卡片、闲鱼重复发卡保护、可控自动发货策略、1 元套餐 1:1 入账、真实小额单严格门。该接口只读源码和当前运行态，不发卡、不点击闲鱼发货、不恢复自动发货；历史真实闲鱼订单严格门已 PASS、补救队列为 0，但自动发货因库存事实源分裂保持暂停保护 |
...
| 生产闭环审计 | `cc_zhongzhuan_readiness_audit.mjs` | `scripts/cc_zhongzhuan_readiness_audit.mjs` | 默认只读检查 Chrome 运营入口、本机闲鱼助手、本机 GUI 状态、本机配置、Oracle 服务/库存/公网安全门；Oracle 合同失败关闭校验 12 个分组/账号/启用渠道、10 条启用文本监控、2 条禁用生图监控、12 条 300±30 秒调度、文本 `+0.05x` 倍率差、生图 `1.0x`/按次 `0.10/0.12` 与渠道A Claude 账号 #2 状态。2026-08-10 真实复核发现审计展示的 807 张仅属于 Sub2API PostgreSQL，实际已付款 webhook 的 Frist-API 旧卡池可分配库存为 0；自动发货因此保持暂停，公开售卖不通过。历史真实闲鱼订单严格门与补救队列为 0 的结论不能覆盖该 P0；不输出 token、卡密或用户 Key |

...
| 生产闭环审计 | `cc_zhongzhuan_readiness_audit.mjs` | `scripts/cc_zhongzhuan_readiness_audit.mjs` | 默认只读检查 Chrome 运营入口、本机闲鱼助手、本机 GUI 状态、本机配置、Oracle 服务/库存/公网安全门；Oracle 合同失败关闭校验 12 个分组/账号/启用渠道、10 条启用文本监控、2 条禁用生图监控、12 条 300±30 秒调度、文本 `+0.05x` 倍率差、生图 `1.0x`/按次 `0.10/0.12` 与渠道A Claude 账号 #2 状态；当前默认与 `--require-real-order` 两种只读巡检均 PASS，可用兑换码 807、自动发货未暂停、历史真实闲鱼订单 1 单、补救队列 0；链动七档商品均为 101 张且销售中，¥30 商品为 100 张且销售中，首笔 ¥1 真实购买仍须单独实单确认；不输出 token、卡密或用户 Key |

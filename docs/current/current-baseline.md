...
> 更新时间：2026-08-16（Asia/Singapore；本轮生产发布记录起于 2026-08-15 UTC）。生产运行时是唯一事实；仓库仅用于必要维护、恢复和异地备份。`docs/current/` 只保留本文件。
...
- GitHub Actions 运行 `31894209937`（分支 `codex/production-final-audit-20260815`）构建门禁全部通过并发布可校验的 Linux ARM64 工件；定时任务对未审阅的官方 `v0.1.176` 已改为安全跳过，手动指定仍明确失败。

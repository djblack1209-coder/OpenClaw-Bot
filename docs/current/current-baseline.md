...
- 本轮新增需求的生产写入仍未执行：云猫公开店铺已确认可达但暂无商品，平台发布入口因店铺联系方式未完善而阻断；本地已准备可配置的 `充值中心2` 代码和回滚边界，未把未部署代码写成生产通过。
...
- 云猫寄售真实登录态只读复查：商家后台可达，商品列表为 0；公开店铺 `https://catfk.com/shop/RJYFBH36` 返回 200 且页面显示“暂无商品”。商品发布动作被平台引导至店铺设置，要求先填写至少一项联系方式；未代填或保存任何联系方式。
...
- 云猫商品仍未上架：平台要求店铺联系方式，且尚未确认七档商品的标题、价格、库存、兑换码来源、手续费承担方和上架确认；这些属于用户/商家侧的外部写入，不能由本地代码推断。
...
- 云猫充值中心2部署：完成店铺联系方式和七档商品参数确认后，按现有 `sub2api_oracle_manage.sh recharge-center` 单一入口部署；预计 30–60 分钟，收益是第二公开充值渠道，维护成本为一个菜单项和一次 CSP 回读。
...
- 用户需处理云猫店铺联系方式、商品/价格/库存与平台保证金或手续费事项；如平台触发 MFA、验证码或手机确认，必须由用户接管。
...
云猫充值中心2：公开店铺页面已确认可达但暂无商品；本地分支已加入 `recharge-center-2` 菜单和专用全屏嵌入，未部署到 Oracle。完成店铺联系方式及七档商品参数确认后，先做生产 prestate/备份，再只执行 `sub2api_oracle_manage.sh recharge-center`，回读菜单、CSP、两个页面和公开商品，失败立即回滚。

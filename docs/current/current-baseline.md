...
- 本轮 Oracle 生产写入仍未执行；云猫商家资料已按链动小铺可见资料补齐，已建立七档下架商品草稿，但未导入库存、未上架销售，未把未完成的库存迁移或未部署的 `充值中心2` 代码写成生产通过。
...
- 云猫寄售真实登录态复查：商家后台可达；已复制链动小铺的店铺昵称、公告、官网按钮和已有联系方式，并保存为买家承担手续费；已建立 `¥1/10/50/100/300/500/1000` 七档 `JIYU AI 余额兑换码` 下架草稿，分类为 `JIYU AI 余额`，每档库存仍为 0。
- 云猫原生跨站搬家检测链动源站成功，但迁移范围包含库存且要求源站账号密码；本轮没有读取、打印或提交密码，没有导入任何卡密正文，浏览器已停在人工确认页。
...
- 云猫商品仍未上架：七档标题、价格、分类、说明和买家承担手续费已保存，但库存/卡密尚未迁移；云猫迁移插件要求把链动账号密码提交给云猫，必须由用户在浏览器中人工确认，不能由本地代码推断或代交密码。
- `充值中心2` 本地代码已提交到维护分支，但尚未在 Oracle 执行 `recharge-center` 生产写入；需要先完成店铺资料和商品参数确认，再生成最新 prestate/备份并回读菜单、CSP、页面和公开店铺。
...
- 云猫充值中心2部署：完成库存迁移并确认七档商品可售后，按现有 `sub2api_oracle_manage.sh recharge-center` 单一入口部署；预计 30–60 分钟，收益是第二公开充值渠道，维护成本为一个菜单项和一次 CSP 回读。
...
- 用户需在已保留的云猫迁移页确认链动源站账号密码并点击“开始迁移”；如平台触发 MFA、验证码、保证金、续费或手机确认，必须由用户接管。迁移完成后再通知我继续库存回读、上架和 Oracle `充值中心2` 部署。
...
云猫充值中心2：云猫商家资料与七档下架商品模板已保存，当前每档库存为 0；库存迁移需用户在浏览器人工确认源站账号密码。完成迁移并确认商品可售后，先做 Oracle 生产 prestate/备份，再只执行 `sub2api_oracle_manage.sh recharge-center`，回读菜单、CSP、两个页面和公开商品，失败立即回滚。

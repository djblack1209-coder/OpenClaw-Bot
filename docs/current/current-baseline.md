...
> 更新时间：2026-08-16（Asia/Singapore；本轮最终复查完成于 2026-08-16T03:42Z，跨项目备份状态于 2026-08-16T05:47Z 收口，渠道A OpenAI Pro 监控恢复于 2026-08-16T10:45Z，营销前成本审计完成于 2026-08-16T11:02Z）。生产运行时是唯一事实；仓库仅用于必要维护、恢复和异地备份。`docs/current/` 只保留本文件。
...
- 本次生产复查没有制造订单、支付或额外合成负载，也没有新增控制面；Oracle 只执行了有备份、可回滚的同版本前端工件发布、充值菜单最小写入，以及已记录的渠道A OpenAI Pro 分组/监控恢复。后者由原有调度器按正常首次检查语义执行一次实际监控。
...
- 营销前成本审计确认 14 个公开分组均有唯一活动账号、没有用户专属费率/RPM 覆盖，且当前售价都高于关联账号成本至少 `0.05` 倍率；但这只是当前快照，不构成全量拉新的成本放行。
...
- 当前 `payment_orders=0`、近 24 小时/7 天 `usage_logs=0`、active channels=9、active accounts=16；10 个文本监控已启用，另有 2 个生图监控因真实上游问题保持停用。
...
- 未读取 Cookie、Local Storage、密码或会话存储，没有创建 Key 或发送人工构造的业务请求。除本次有备份和 prestate 的渠道A OpenAI Pro 分组/监控恢复外，没有提交生产表单或刷新配置。
...
- 全量营销当前不放行：近 30 天的 10 条真实计费记录全部发生在 2026-08-08，其中渠道B Claude 官 Key 有 4 条实际成本高于用户计费的历史亏损记录；之后没有新的真实计费样本验证该路径。渠道B的 Claude/OpenAI 四个分组成本源不支持自动倍率刷新，两个 Grok 分组又因启用生图能力被文本费率同步排除；当前价差均为正，但不能自动吸收上游成本变化。
- 两条生图上游仍受权限拒绝或目标模型缺失阻断；未制造付费探针，也未恢复中央 MCP。
- 上游渠道页仍有错误/降级模型结果：当前 3 条 Claude 监控报错、2 条 Grok 监控降级，10 条启用监控虽都新鲜但不应作为对外全量广告卖点。需供应商恢复、限流解除或运营决定停用对应报价；本项目没有可安全替代的生产修复。
...
- 广告投放前必须先由运营选择一个可逆的可售范围：要么暂时下架/隐藏成本不可自动刷新或当前异常的公开分组，只保留已验证的健康目录；要么明确接受静态成本风险、指定人工成本复核责任和亏损阈值。完成选择及最新 prestate/备份后的最小变更回读前，不做全量拉新。
...
生产回读：7 个符合条件的受管文本 group 正确、漂移为 0；10 个文本监控启用，其中渠道A OpenAI Pro 仅保留 1 条、已完成恢复后的实际检查；渠道页仍有真实上游错误/降级结果；两条生图上游仍受权限/模型阻断。供应商确认恢复后只做一次最低成本真图验收，不恢复中央 MCP。
...
渠道A OpenAI Pro 的 V1 监控当前仅有 1 条且启用。若再次显示停用，先只读核对其 channel/group/account 是否全为 active；绑定失效时的自动停用是预期保护，不能靠重复点击开关绕过。任何恢复均先做最新 Sub2API 备份和 prestate，再恢复绑定、启用原记录并通过原生编辑路径重新登记调度；不要创建或保留重复监控。

营销前成本结论：14 个公开分组当前均为单活动账号、无用户费率/RPM 覆盖，所有当前分组倍率都至少高于关联账号成本 `0.05`；自动成本刷新只对 5 个来源返回 fresh `ok`，另 5 个来源为 `unsupported`，且两个 Grok 组因允许生图不参与文本费率同步。2026-08-08 的渠道B Claude 官 Key 曾出现 4 条账本亏损，此后没有真实计费样本；当前还有 3 条 Claude 监控 error、2 条 Grok degraded。禁止给全量广告放行，先取得运营对可售目录收缩或静态成本人工复核的明确决定，再做带备份/prestate/回滚的最小生产改动。
```

...
> 更新时间：2026-08-16（Asia/Singapore；本轮最终复查完成于 2026-08-16T03:42Z，跨项目备份状态于 2026-08-16T05:47Z 收口，渠道A OpenAI Pro 监控恢复于 2026-08-16T10:45Z，营销目录、成本口径与备份收敛于 2026-08-16T11:30Z）。生产运行时是唯一事实；仓库仅用于必要维护、恢复和异地备份。`docs/current/` 只保留本文件。
...
- 本次生产复查没有制造订单、支付或额外合成负载，也没有新增控制面；Oracle 只执行了有备份、可回滚的同版本前端工件发布、充值菜单最小写入、渠道A OpenAI Pro 恢复，以及营销目录的可逆收敛。营销目录变更只把 12 个未同时满足成本自动回读与当前稳定性的公开分组改为 `inactive`，账号、渠道、费率、绑定、模型与历史数据均保留。
...
- 本机严格健康当前为 `ok=true/release_ready=false/bad=0/warn=1`；唯一 warning 是 daily-backup LaunchAgent 的 `runs=0/(never exited)` 尚未产生下一次 03:30 自然运行证据。最新本机备份新鲜度和本轮手工备份恢复演练均通过，不代表 JIYU 生产故障，也不阻塞只读诊断或真实业务修复。
- 营销前账本口径已纠正：`actual_cost` 是实际向用户扣减的金额，`total_cost` 是倍率前基准价，账号侧预估成本按 `total_cost × account_rate_multiplier` 计算。近 30 天 10 笔真实用量的用户扣费为 `$0.05253097`、账号侧预估成本为 `$0.04588517`、预估毛利为 `$0.00664580`，负毛利 0 笔；先前把 `actual_cost > total_cost` 解释为亏损属于错误比较。
- 当前仅 `JIYU OpenAI Plus · 渠道A` 与 `JIYU OpenAI Pro · 渠道A` 两个公开分组保持 `active`。两者均为唯一活动账号、利润控制开启、无用户专属倍率/RPM 覆盖、账号费率同步开启、探测状态 fresh `ok`，且分组倍率恰为账号倍率 `+0.05`。广告可开始，但卖点和引导只能覆盖这两项，不得把已停用目录描述为可用。

## 1. 已通过的真实生产检查

### 本机 Mac

- `scripts/auto_health_check.sh --json --strict` 返回 `ok=true`、`release_ready=false`、`bad=0`、`warn=1`；核心服务、备份新鲜度、ClawBot API、Gateway API、公网站点和上游监控均通过，唯一 warning 是 daily-backup 等待下一次自然 LaunchAgent 产物。scheduler 的生产周期产物、标准输出和投递结果仍已验证。
...
- 当前 `payment_orders=0`、近 24 小时/7 天 `usage_logs=0`、active accounts=16；营销目录收敛后只有 2 个公开分组 active，其余 12 个公开文本分组为 inactive、2 个生图分组为 disabled。正常调度周期已将 8 条对应文本监控自动停用；加上原有 2 条生图监控，最终为 2 条启用、10 条停用，没有人工制造额外探测。
- 两个当前可售分组的账号倍率与分组倍率分别为 `0.07 → 0.12`、`0.10 → 0.15`；费率同步、利润控制和 fresh 探测均开启，合同漂移为 0。
- 备份目录既有 38/38 checksum 校验通过，备份 timer active；营销目录收敛后新增 Oracle 一致性备份 `/var/backups/sub2api/daily-20260816T113037Z` 和本机归档 `openeverything-20260816-193034.tgz`。本机归档的 checksum、路径、manifest 与 SQLite 只读恢复演练通过；清单中 4 个当前不存在的运行目录被如实标记，离机加密介质仍未配置。

### 真实 Chrome 登录态与窄屏回读

- 已接管现有登录态只读打开仪表盘和渠道状态页，页面可达并已恢复到原仪表盘位置。
- 营销目录收敛前的渠道状态页确有持续错误/降级：渠道A Claude Kiro 在 24 小时内 290/292 次 error、渠道B Claude Kiro 292/292 次 error，两条 Grok 多数为 degraded；这些结果是停用相应报价的依据，不能用总健康端点掩盖。
- 收敛后的真实渠道状态页总体为 `OPERATIONAL`，只显示渠道A OpenAI Plus/Pro；自 10:45Z 起两者分别 9/9 次 operational。页面保留真实 7 天历史，当前仍显示 Plus `91.29%`、Pro `30.51%`，因此只允许小规模启动并观察，不对外承诺 SLA，也不删除修复前故障记录。
...
- 未读取 Cookie、Local Storage、密码或会话存储，没有创建 Key 或发送人工构造的业务请求。营销目录收敛通过原生分组编辑表单提交 12 次状态更新，审计日志为 12 条 `admin.groups.update`/HTTP 200；用户创建 Key 的真实下拉框只显示渠道A OpenAI Plus/Pro 两项，未提交创建。
...
- 2026-08-16 恢复渠道A OpenAI Pro：根因是其绑定分组处于停用，而渠道和账号均正常。受管监控的通用保护会在 channel/group/account 任一失效时自动停止监控，不存在按“渠道A Pro”名称硬编码的开关。先用原生后台恢复分组，再核对数据库绑定；过程中发现一条意外的停用重复监控，已用新备份和 prestate 精确删除重复项、保留原记录并启用。随后经原生编辑保存重新登记调度，唯一记录、启用状态、实际检查、服务健康和无“绑定失效”事件均已回读通过。
- 纠正营销前成本审计公式：旧查询把用户扣费 `actual_cost` 与倍率前基准价 `total_cost` 比较，错误标记出 4 条“亏损”。按生产计费代码的真实语义改用 `actual_cost - total_cost × account_rate_multiplier` 后，10/10 笔均为正毛利，最小单笔预估毛利 `$0.00009780`。
- 在一致性备份 `/var/backups/sub2api/daily-20260816T112114Z` 和 14 个公开分组的最新 prestate 后，只将 12 个未同时满足“成本自动回读 + 当前稳定可用”的分组改为 inactive。生产审计确认 12/12 次原生更新均为 HTTP 200、未产生订单或用量；用户真实创建 Key 下拉框只剩两项已验证产品。

## 3. 未修复问题及原因

- 广告可以开始，但当前只放行渠道A OpenAI Plus/Pro。渠道B的 Claude/OpenAI 成本源不支持自动倍率刷新；两个 Grok 因包含图片/视频模型而被文本费率同步有意排除；国内四组没有同等级监控且利润控制未开启。这 12 个分组已停用，只有在供应商恢复、成本来源可验证并按最新 prestate/备份完成真实回读后才能逐项恢复。
- 两条生图上游仍受权限拒绝或目标模型缺失阻断；未制造付费探针，也未恢复中央 MCP。
- 停用目录的历史错误/降级仍属于真实供应商问题，不以改超时、改路由或增加重试掩盖。对应账号、渠道和历史监控保留以便恢复取证，但不再进入新用户可选目录。
...
- 当前没有阻止开始广告的 P0；放行范围严格限定为渠道A OpenAI Plus/Pro。任何宣传全目录、Claude、Grok、国内四组或生图的内容都会重新引入 P0，必须先恢复并验证对应生产路径。

### P1

- 在既有 VPS-Config 能力内加入 JIYU `/health` 外部失败通知，目标发现时间 1–5 分钟；不在本仓新增 watcher。
- 观察渠道A OpenAI Pro 连续 24 小时监控样本，并跟踪渠道A OpenAI Plus 24 小时 operational 比例；当前目录可小规模拉新，不据此承诺 SLA。
- 对已停用的上游报价只做供应商侧修复和逐项恢复，不因广告扩量一次性启用全目录。
...
- 本地减负后的 OpenClaw `health` 和 `doctor --lint` 通过；当前严格健康仅因 daily-backup 等待下一次自然调度而为 `release_ready=false`，没有 bad 项。清理仅涉及可再生、被 Git 忽略的构建/测试产物。
...
- 营销目录写入前生成 Sub2API 一致性备份 `/var/backups/sub2api/daily-20260816T112114Z` 并回读 14 个公开分组 prestate；写入后审计日志只有 12 条预期的分组更新，新增订单和用量均为 0。真实用户创建 Key 下拉框只剩渠道A OpenAI Plus/Pro，未提交创建。
...
生产回读：当前公开可售目录只有渠道A OpenAI Plus/Pro 两个 group。两组均为唯一活动账号、无用户专属倍率/RPM 覆盖、利润控制开启、费率同步开启、账单探测 fresh `ok`，分组倍率分别为 `0.12/0.15`、均等于账号倍率 `+0.05`；用户创建 Key 下拉框只显示这两项。其余 12 个公开文本分组为 inactive，两条生图分组为 disabled；账号、渠道、费率、绑定、模型和历史数据均保留。

本机严格健康当前返回 `ok=true/release_ready=false/bad=0/warn=1`；唯一 warning 是 daily-backup 的 `runs=0/(never exited)` 等待下一次 03:30 自然运行。最新备份新鲜度、本轮手工归档和恢复演练已通过；不要 kickstart 或重载任务来追求计数器变化。
...
闲鱼、卖家桥、自动交付、Frist-API、CC 中转和中央生图 MCP 已退役，不要恢复。支付、订单、授权、数据库迁移、备份恢复、安全和关键浏览器测试必须保留。当前没有阻止小范围投放的 P0，但广告和用户引导只能覆盖渠道A OpenAI Plus/Pro；优先观察真实转化和两项监控，不得因扩量一次性恢复全目录。

渠道A OpenAI Pro 的 V1 监控当前仅有 1 条且启用。若再次显示停用，先只读核对其 channel/group/account 是否全为 active；绑定失效时的自动停用是预期保护，不能靠重复点击开关绕过。任何恢复均先做最新 Sub2API 备份和 prestate，再恢复绑定、启用原记录并通过原生编辑路径重新登记调度；不要创建或保留重复监控。其他文本监控随对应分组 inactive 已由现有调度器自然停用，不应手工重新打开。

营销成本结论：上一轮“渠道B Claude 官 Key 有 4 条亏损”的判断已撤销，原因是把用户扣费 `actual_cost` 与倍率前基准价 `total_cost` 混比。正确公式 `actual_cost - total_cost × account_rate_multiplier` 对近 30 天 10/10 笔均为正，合计预估毛利 `$0.00664580`。现在允许开始广告，但范围只限渠道A OpenAI Plus/Pro；Claude、Grok、国内四组和生图均未放行，恢复前必须重新满足当前成本与真实可用性检查。
```

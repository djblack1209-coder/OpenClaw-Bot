...
> 当前状态: 只说明原生能力与启用条件；支付、对象存储、异步生图和地区分流均未因本文自动启用。
...
当前对象存储的访问权限和可用凭据尚未满足启用条件，因此异步生图保持关闭。不要将 Access Key、Secret、Bucket 名称、账户标识或回调密钥写入聊天、文档、命令历史或仓库。配置完成前也不要创建付费测试批次。
...
目标规则是：中国大陆 IP 只显示国内模型；境外 IP 显示全部模型；用户、余额、订单和密钥仍由海外主系统作为唯一事实来源。

最小架构是使用 Cloudflare 边缘层做地区判断与服务端目录过滤，将账号、账本、支付、API 鉴权和模型请求继续保留在海外主系统。国内节点如后续启用，只承担轻量页面或目录能力；模型流式数据不应经由国内节点转发，从而避免重复账本和大流量中转。

当前只读核对已确认海外 SSH、Cloudflare CLI 登录态和生产备份可用；法律文案修正可由现有 Sub2API 管理脚本回滚。技术分流尚未完成可回滚的生产发布，也没有确认对象存储或边缘权限满足配置条件，因此仍是待实施项。上线前必须用中国大陆和境外的真实访问条件回读页面和 API，确认既不会漏出受限模型，也不会分裂余额。

...
### 文件变更
- `packages/clawbot/src/xianyu/wechat.py` — 编号命令系统
- `packages/clawbot/src/api/rpc.py` — 导入修复
- `packages/clawbot/src/monitoring/world_monitor.py` — yfinance getattr 修复
- `scripts/social_browser_worker.py` — Chrome 内存优化 + 标签页清理
- `multi_main.py` — 懒加载
- 前端 8 个组件 — 轮询优化/错误处理/变量遮蔽修复

## 2026-04-24 — 全量审计第十轮：any 类型债首批收敛
> 领域: `frontend` `docs`
> 影响模块: `Store`, `Bots`, `Social`, `Xianyu`, `AIConfig`
> 关联问题: AUDIT-2026-04-24-R35~R38

### 变更内容
- Store 修复 unused eslint-disable 和未使用变量。
- Bots/Social/Xianyu/Store 共 8 处 `catch (e: any)` 改为 `catch (e: unknown)` + `(e as Error)?.message`。
- AIConfig 8 处 `as any` 改为 `as PoolStats` 或 `Record<string, unknown>`，利用已有 index signature 去掉冗余 cast。
- 前端 ESLint warning 从 82 降到 65。

### 文件变更
- `apps/openclaw-manager-src/src/components/Store/index.tsx` — 修复 unused eslint-disable + catch 类型。
- `apps/openclaw-manager-src/src/components/Bots/index.tsx` — catch 类型 + error 访问修正。
...
> 领域: `frontend` `docs`
> 影响模块: `Channels`, `Dev`, `DevPanel`, `WorldMonitor`, `APIGateway`, `Evolution`, `Logs`, `Memory`, `Money`, `Onboarding`, `Performance`, `Portfolio`, `Scheduler`, `Setup`
> 关联问题: AUDIT-2026-04-24-R32~R34

### 变更内容
- 第八轮：修复 Channels/Dev/DevPanel/WorldMonitor 数据拉取和计算回调缺少翻译函数依赖，lint 103→96。
- 第九轮：一次性修完全部剩余 14 处 Hook 依赖（APIGateway 令牌删除、Evolution/Logs/Money/Performance/Scheduler/Setup 数据拉取、Memory 搜索、Onboarding 完成、Portfolio 持仓和交易控制拉取、WorldMonitor 冲突区域计算），lint 96→82。
- **里程碑**：`react-hooks/exhaustive-deps` 从首轮 132 个降为 0，前端 Hook 闭包风险归零。
- 剩余 82 个 warning 全部为 `@typescript-eslint/no-explicit-any` 类型债。
...
> 领域: `frontend` `docs`
> 影响模块: `AIConfig`, `APIGateway`, `Bots`, `NewsFeed`
> 关联问题: AUDIT-2026-04-24-R27~R31

### 变更内容
- 修复 AIConfig 数据拉取和渠道切换 Hook 依赖，避免渠道占位名和提示文案使用旧语言。
- 修复 APIGateway 渠道切换/删除、令牌删除、网关启停等 Hook 依赖，降低确认弹窗和 toast 旧状态风险。
- 修复 Bots 数据拉取 Hook 依赖，避免加载失败提示使用旧语言。
- 修复 NewsFeed 新闻拉取、热门话题、AI 摘要 Hook 依赖，避免文章数量、摘要空态等文案旧语言。
- 前端 ESLint warning 从 112 降到 103。

### 文件变更
- `apps/openclaw-manager-src/src/components/AIConfig/index.tsx` — 补齐数据拉取和渠道切换依赖。
...
> 领域: `frontend` `docs`
> 影响模块: `Assistant`, `Xianyu`, `Social`, `Settings`
> 关联问题: AUDIT-2026-04-24-R23~R26

### 变更内容
- 修复 AI 助手会话加载、选择、创建、删除、发送消息、附件上传、语音录制等回调缺少翻译函数依赖的问题。
- 修复闲鱼数据刷新和扫码登录轮询相关 Hook 依赖，降低语言切换后提示文案不更新的风险。
- 修复社媒和设置页核心数据拉取/操作回调缺少翻译函数依赖的问题。
- 前端 ESLint warning 从 123 降到 112；剩余主要是内部工具页 Hook 依赖和 `any` 类型债。

### 文件变更
- `apps/openclaw-manager-src/src/components/Assistant/index.tsx` — 补齐核心交互回调依赖。
...
> 领域: `frontend` `docs`
> 影响模块: `App`, `CommandPalette`, `Header`, `Sidebar`, `Home`, `FinRadar`
> 关联问题: AUDIT-2026-04-24-R19~R22

### 变更内容
- 修复命令面板自然语言执行闭包缺少翻译函数依赖的问题，并收紧导航参数类型。
- 修复 App 环境检查和服务状态轮询 Hook 依赖，降低旧状态/旧 action 引用风险。
- 修复 Header 时钟语言、Sidebar 开发者模式三击、Home 首页日志时间/错误文案、FinRadar 其他标签等用户可见状态的 Hook 依赖。
- 前端 ESLint warning 从 129 降到 123；剩余主要是跨页面 `any` 类型和其他 Hook 依赖技术债。

### 文件变更
- `apps/openclaw-manager-src/src/App.tsx` — 补齐环境检查、服务轮询 Hook 依赖。
- `apps/openclaw-manager-src/src/components/CommandPalette.tsx` — 补齐命令执行 Hook 依赖，收紧导航类型。
- `apps/openclaw-manager-src/src/components/Layout/Header.tsx` — 补齐时钟语言依赖。
...
> 领域: `frontend` `docs`
> 影响模块: `App.tsx`, UI 审计截图链路
> 关联问题: AUDIT-2026-04-24-R15~R18

### 变更内容
- 为 Tauri/React 单页应用增加受控 `?page=<PageType>` 深链接能力，解决内部状态切页无法自动逐页截图的问题。
- 增加开发者页面守卫：未开启开发者模式时，`?page=devpanel`、`?page=gateway` 等开发者页深链接会回落到首页。
- 使用深链接生成 30 个页面截图基线，覆盖主页面和开发者页面。
- 登记当前截图工具无法自动做肉眼级视觉判断的剩余风险。
...
> 领域: `frontend` `docs`
> 影响模块: `CommandPalette`, 桌面端主导航
> 关联问题: AUDIT-2026-04-24-R12~R14

### 变更内容
- 修复 Ctrl+K 命令面板仍只暴露旧开发者导航的问题。
- 命令面板新增与侧边栏一致的主功能入口：主页、AI 助手、全球监控、新闻、金融雷达、投资组合、机器人、商店、闲鱼、社媒、设置。
- 保留开发者导航，但单独放到“开发者工具”分组，减少普通用户误入内部页面。
- 本轮尝试批量截图审计，确认当前 Playwright CLI 截图只能重复打开首页，无法覆盖应用内状态切页，已登记继续审计项。
...
> 影响模块: `i18n`, `docker-compose.yml`, `.env.example`
> 关联问题: AUDIT-2026-04-24-R1

### 变更内容
- 修复前端 i18n 文案中的多余转义字符，解除 ESLint 的 4 个 `no-useless-escape` 错误。
- 加固 Docker Compose Redis 配置，未显式提供 `REDIS_PASSWORD` 时直接失败，避免空密码 Redis 在生产环境启动。
- `.env.example` 补充 `REDIS_PASSWORD` 配置说明，降低部署时漏配风险。

### 文件变更
- `apps/openclaw-manager-src/src/i18n/zh-CN.ts` — 去除无效转义。
- `apps/openclaw-manager-src/src/i18n/en-US.ts` — 去除无效转义。
- `docker-compose.yml` — Redis 密码改为强制环境变量。
- `packages/clawbot/config/.env.example` — 新增 Redis 密码配置项。

## 2026-04-24 — 全量审计第二轮：后端测试恢复 + 微信 API 路由回归修复
> 领域: `backend` `frontend` `infra` `docs`
> 影响模块: `api/routers/wechat.py`, Python 测试环境, Node 22 前端构建链, Tauri 入口
> 关联问题: AUDIT-2026-04-24-R8~R11

### 变更内容
- 恢复缺失的微信 API 路由 `packages/clawbot/src/api/routers/wechat.py`，修复 `router_wechat` 导入崩溃。
- 微信 `/api/v1/wechat/incoming` 改用 Pydantic request/response model，避免裸 dict 请求体，和 FastAPI 当前推荐模式对齐。
- 建立 Python 3.12 `.venv312` 测试环境并安装 `requirements-dev.txt`，后端全量 pytest 跑通。
- 切换 Node 22.22.2 后重跑前端依赖安装、类型检查、生产构建，确认 Node 版本问题已消除。
- 验证 Tauri CLI 2.10.1、Rust `cargo check`、`/Applications/OpenClaw.app` 存在且旧 `OpenEverything.app` 不存在。
...
> 领域: `backend`
> 影响模块: `llm_cache.py`, `litellm_router.py`, `multi_main.py`, `utils_cache.py`
> 关联问题: HI-388 (CVE-2025-69872)

### 变更内容

- 新增 `src/utils_cache.py`: 基于 sqlite3 标准库的 DiskCache 类，零外部依赖
  - 接口兼容 diskcache.Cache 常用方法: get/set(expire=)/len/volume/close/clear
  - 支持 TTL 自动过期、LRU 淘汰、pickle 序列化复杂对象
  - WAL 模式 + threading.Lock 保证并发安全
- 修改 `src/llm_cache.py`: `import diskcache` → `from src.utils_cache import DiskCache`
- 修改 `src/litellm_router.py`: 降级方案从 `import diskcache` 改为 `from src.utils_cache import DiskCache`
- 修改 `multi_main.py`: 更新缓存关闭时的注释
- 从 `requirements.txt` 移除 `diskcache~=5.6.0` 依赖

### 文件变更
- `src/utils_cache.py` — 新增，自研磁盘缓存模块
- `src/llm_cache.py` — 替换 diskcache 导入
...
> 领域: `backend`, `frontend`, `ai-pool`
> 影响模块: `rpc`, `ws`, `conversation`, `trading`, `omega`, `evolution`, `newapi`, `proactive_listeners`, `brain`, `proactive_engine`, `litellm_router`, `trading_pipeline`, `pydantic_agents`, 前端 `conversationService`/`api`
> 关联问题: AUDIT_PLAN v3.0 R2, HI-NEW-01/02/03

### 变更内容

**R2 审计: 46 条目 (28 通过 / 14 修复 / 5 技术债)**

**修复汇总（14项）**:
1. **rpc.py**: `import os` 移到顶部，修复 `_rpc_system_status()` NameError (HI-NEW-01)
2. **ws.py**: 完全重写 WebSocket — 共享deque→每客户端独立asyncio.Queue + 初始状态try/except保护 (HI-NEW-02/03)
3. **conversation.py**: message 从 Query string 改为 Body（防用户聊天内容泄露到日志）+ 会话上限200防DoS
4. **trading.py**: sell_position/add_to_watchlist 从 request.json() 改为 Pydantic model（SellRequest/WatchlistAddRequest）
5. **omega.py**: message Query→Body + except HTTPException: raise 防 generic except 吞掉
6. **evolution.py**: update_proposal 加 except HTTPException: raise
7. **newapi.py**: 空 NEWAPI_ADMIN_TOKEN 时提前拒绝请求（返回503）
8. **proactive_listeners.py**: 修复5处 `await bus.subscribe(...)` — subscribe() 是同步方法，5个事件监听器静默注册失败
9. **brain.py**: resume_with_answer/handle_callback 错误路径补充 _scrub_secrets 防密钥泄露
10. **proactive_engine.py**: evaluate() 入口调用 _cleanup_old_entries() 防内存泄漏
11. **litellm_router.py**: iflow key fingerprint 从明文前缀改为 SHA256 hash（防日志泄露完整key）
12. **trading_pipeline.py**: 3处 `except Exception: pass` 改为 `logger.debug`（交易管道静默异常=资金风险）
13. **pydantic_agents.py**: iflow key 从模块级固化（IFLOW_KEY常量）改为 @property 实时读取环境变量（key 7天过期后不再需要重启进程）
14. **前端同步**: conversationService.ts + api.ts 的 conversationSend 改为 JSON body 传参

**技术债登记（5项）**:
- R2.25/R2.29: config/llm_routing.json 与 litellm_router.py 硬编码完全漂移（URL和模型名）
- R2.36: 无数据库自动备份策略
- R2.44: 代码库59个静默异常（最危险3个已修复）
- R2.45: 8个 create_task 未加 done_callback（幽灵任务风险）

### 文件变更
- `packages/clawbot/src/api/rpc.py` — 加 import os
- `packages/clawbot/src/api/routers/ws.py` — 完全重写
- `packages/clawbot/src/api/routers/conversation.py` — message Query→Body + 会话上限
- `packages/clawbot/src/api/routers/trading.py` — Pydantic model
- `packages/clawbot/src/api/routers/omega.py` — message Body + HTTPException
- `packages/clawbot/src/api/routers/evolution.py` — HTTPException
- `packages/clawbot/src/api/routers/newapi.py` — 空token拒绝
- `packages/clawbot/src/core/proactive_listeners.py` — await→sync
- `packages/clawbot/src/core/brain.py` — _scrub_secrets
- `packages/clawbot/src/core/proactive_engine.py` — cleanup
- `packages/clawbot/src/litellm_router.py` — SHA256 hash
- `packages/clawbot/src/trading_pipeline.py` — logger.debug
- `packages/clawbot/src/modules/investment/pydantic_agents.py` — key property
- `apps/openclaw-manager-src/src/services/conversationService.ts` — JSON body
- `apps/openclaw-manager-src/src/lib/api.ts` — JSON body

---

## 2026-04-18 — 全方位审计 v3.0 R1: 基础设施与文件治理
> 领域: `infra`, `docs`
> 影响模块: `.gitignore`, `docker-compose.yml`, `requirements.txt`, `pytest.ini`, CI/CD, 文档体系
> 关联问题: AUDIT_PLAN v3.0 R1

### 变更内容

**审计方案设计**: 创建 11 轮 425+ 条目的精细化审计方案（docs/audit/R01-R11），覆盖基础设施/后端/Bot/macOS/交易/社媒/运维/E2E

**R1 修复汇总（6项）**:
1. **删除过时脚本**: `cleanup-redundant.sh`（目标目录已不存在）
2. **创建 `.secrets.baseline`**: 修复 pre-commit detect-secrets hook 缺失
3. **删除冗余 CI**: `packages/clawbot/.github/`（monorepo 子目录 CI 不触发）
4. **修复 Redis 配置**: maxmemory 从 256mb 调整为 100mb（匹配容器 128M 限制）
5. **加固依赖安全**: RestrictedPython 添加 `<9.0` 上限 + DEPENDENCY_MAP stamina 版本对齐
6. **优化 pytest.ini**: 添加 `-x --tb=short -q` 默认选项和 DeprecationWarning 过滤
7. **清理废弃文档**: 删除 `docs/superpowers/` 旧审计方案（已被 v3.0 替代）

### 文件变更
- `cleanup-redundant.sh` — 删除
- `.secrets.baseline` — 新增
...
- `docs/audit/R01-R11_*.md` — 11 个审计条目文档新增
...
## 2026-04-18 — 提示词 H: Tauri 侧边栏导航重构 (TradingView 风格)
> 领域: `frontend`
> 影响模块: `Sidebar`, `Layout`
> 关联问题: GUI 重构 Sprint 提示词 H

### 变更内容
1. **开发者菜单语义分组**: 16 个开发者菜单项从扁平列表重构为 3 个语义分组：
   - 系统管控（总控中心/概览/API 网关/任务调度）
   - 业务运营（消息渠道/社媒总控/盈利总控/AI 配置/MCP 插件/记忆脑图）
   - 开发调试（智能流监控/进化引擎/开发总控/开发者工作台/测试诊断/应用日志）
2. **分组标签 + 分隔线**: 每组有分类标签（折叠态显示短横线），组间有语义分隔线
3. **全面 Design Token 化**: 所有颜色从硬编码 `dark-*` / `var(--oc-*)` 统一为 `var(--bg-*)`, `var(--text-*)`, `var(--border-*)`, `var(--brand-*)` Design Token CSS 变量
4. **TradingView 风格激活指示器**: 左侧指示条改为 3px 宽青色细条 + 10% 透明度背景高亮
5. **hover 交互增强**: 添加 `group` hover 父子联动，图标和文字颜色分级过渡
6. **折叠态优化**: 折叠时分组标签变为居中短横线，保持视觉分隔

### 文件变更
- `apps/openclaw-manager-src/src/components/Layout/Sidebar.tsx` — 重构为分组导航 + Design Token 化（237 行 → 245 行）

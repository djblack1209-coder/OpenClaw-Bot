...
> 最后更新: 2026-04-25 (内存优化: Chrome 浏览器 + 模块懒加载)

---

## 🟢 2026-04-25 内存优化: Chrome 浏览器 + 模块懒加载

> 用户反馈全服务运行时卡顿。分析发现 Chrome 社交浏览器独占 2.3GB（89%），12 个进程 + 10 个标签页。

### 修复前内存分布
| 组件 | 内存 | 占比 |
|------|------|------|
| Chrome 浏览器进程 (12个) | ~2,300 MB | 89% |
| Python 主进程 (multi_main) | ~119 MB | 5% |
| Tauri 桌面端 | ~42 MB | 2% |
| Python xianyu/g4f | ~30 MB | 1% |
| **总计** | **~2,582 MB** | |

### 已修复
| # | 问题 | 严重度 | 修复方式 | 预计节省 |
|---|------|--------|----------|----------|
| PERF-MEM-001 | Chrome V8 堆 512MB 过大 + 无渲染进程限制 | 🟠 | V8 堆降至 128MB + renderer-process-limit=3 + 禁用冗余功能 | ~800-1200 MB |
| PERF-MEM-002 | Chrome 标签页累积不清理（10个标签页含重复和 blob:） | 🟡 | 新增 cleanup_excess_tabs() 自动清理，上限 4 个 | ~200-400 MB |
| PERF-MEM-003 | browser-use/CrewAI/进化引擎启动时预加载 | 🟡 | 改为懒加载，首次使用时才初始化 | ~30-50 MB |

### 已知残留
| # | 问题 | 分类 | 严重度 | 说明 |
|---|------|------|--------|------|
| PERF-MEM-004 | browser_profiles/openclaw_social 磁盘占用 4.4GB | PERF | 🟡 | Chrome 用户数据目录（缓存/cookie/历史），不影响 RAM 但占磁盘 |

---

## 🟢 2026-04-24 OpenCode GPT5.5 上下文安全线修复

> 本轮修复 OpenCode 总 token 到 194k 左右回答停止的问题。用户确认上下文限制应按 272k 判断，因此不改默认模型，只修正 GPT5.5 的安全上下文声明和 GPT 模式识别。

### 已修复
| # | 问题 | 严重度 | 修复方式 |
|---|------|--------|---------|
| OPENCODE-CTX-194K | OpenCode 会话在 194k token 附近停止回答 | 🟠 | 保持默认模型不变，把 GPT5.5 安全上下文声明调为 272k/input 232k/output 40k |
| OPENCODE-GPT-MODE | GPT 独立模式插件仍识别旧 `gpt-5.4` | 🟡 | 改为识别 `gpt-5.5`，仅手动选择 GPT5.5 时生效 |
...
| GPT5.5 280k 实测 | 🟡 不健康 | HTTP 200 但 completion 为 0，耗时 92s，佐证 272k 附近应作为风险边界 |
...
## 🟢 2026-04-24 全量审计第十二轮：CI 全绿 + API 路由注册验证

> 本轮提交推送所有审计改动，确认远程 CI 全绿，并验证后端 130 个 API 路由全部正确注册。

### 验证结果
| 项目 | 结果 | 说明 |
|------|------|------|
| Git 推送 | ✅ 已推送 | 自动提交插件已将所有改动推送到 `origin/main` |
| CI `frontend-typecheck` | ✅ success | 前端类型检查通过 |
| CI `python-tests` | ✅ success | 后端全量 pytest 通过 |
| CI 整体 | ✅ success | Run #24905169720 全绿 |
| 后端 API 路由注册 | ✅ 130 路由 | `APIServer._configure_app()` 无导入错误，含恢复的 `/api/v1/wechat/incoming` |

### 仍需继续
| # | 问题 | 严重度 | 说明 |
|---|------|--------|------|
| AUDIT-2026-04-24-R40 | 后端 API 真实请求 smoke test 未执行 | 🟡 | 路由注册通过 ≠ 请求不报错，需启动完整服务后用 curl 逐路由打一遍 |
| AUDIT-2026-04-24-R41 | 服务器部署验证、Bot 消息测试、肉眼视觉 QA 仍未覆盖 | 🟡 | 这些依赖真实运行环境和用户凭据 |

---

## 🟡 2026-04-24 全量审计第十一轮：any 类型批量替换评估

> 本轮尝试对剩余 65 个 `any` 做批量 `Record<string, unknown>` 替换，结果引入 20+ 个级联类型错误。根因：每个 `any` 背后是一个未建模的后端 API 返回值，简单换 `unknown` 会导致后续属性访问全部报错。已全部回退到编译通过状态。

### 结论
- 剩余 65 个 `any` 不是"能一步替换的技术债"，而是"需要逐个建模后端 API 返回 interface 的架构债"。
- 这属于"海洋级"重构，每个 API 需要核查实际返回结构、建立 TypeScript interface、同步 Python 端文档。
- 不适合混在审计里批量做，应拆成独立 feature branch 逐个 API 处理。

### 验证结果
| 项目 | 结果 | 说明 |
|------|------|------|
| 批量替换 any | ❌ 回退 | 引入 20+ 类型错误，不是安全改动 |
| 回退后类型检查 | ✅ 通过 | `fnm use 22.22.2 && npx tsc --noEmit` 无错误 |
| 回退后 lint | ✅ 65 warning | 与第十轮结束时一致，无退化 |

### 建议
| # | 后续工作 | 优先级 | 说明 |
|---|---------|--------|------|
| AUDIT-2026-04-24-R39 | 为后端 API 建立 TypeScript 返回值 interface | 🟡 | 建议按"用户使用频率"排序，先建 Assistant/Portfolio/Bots/Settings 的 API 返回 interface，再覆盖内部工具页 |
...
## 🟢 2026-04-24 全量审计第七轮发现与修复

> 本轮继续处理内部工具页和数据页的 `react-hooks/exhaustive-deps`，覆盖 AIConfig、APIGateway、Bots、NewsFeed 等页面。

### 已修复
| # | 问题 | 严重度 | 修复方式 |
|---|------|--------|---------|
| AUDIT-2026-04-24-R27 | AIConfig 数据拉取、渠道切换回调缺少翻译函数依赖 | 🟡 | 补齐 `t` 依赖，避免渠道占位名和 toast 使用旧语言 |
| AUDIT-2026-04-24-R28 | APIGateway 渠道切换/删除、令牌删除、服务启停回调依赖不完整 | 🟡 | 补齐/修正相关 Hook 依赖，避免确认框与 toast 使用旧语言 |
| AUDIT-2026-04-24-R29 | Bots 数据拉取回调缺少翻译函数依赖 | 🟡 | 补齐 `t` 依赖，避免加载失败提示使用旧语言 |
| AUDIT-2026-04-24-R30 | NewsFeed 新闻拉取、热门话题、AI 摘要 Hook 依赖不完整 | 🟡 | 补齐/修正 `t` 依赖，避免新闻时间/摘要/文章数文案旧语言 |
...
| 前端类型检查 | ✅ 通过 | `fnm use 22.22.2 && npx tsc --noEmit` 无错误 |
| 前端生产构建 | ✅ 通过 | `fnm use 22.22.2 && npm run build` 成功 |
| 前端 lint warning | 🟡 下降 | 从 112 降到 103 |
| Diff 检查 | ✅ 通过 | `git diff --check` 无输出 |

### 仍需继续
| # | 问题 | 严重度 | 说明 |
|---|------|--------|------|
| AUDIT-2026-04-24-R31 | 前端仍有 103 个 lint warning | 🟡 | 剩余 Hook 依赖集中在 Channels/Dev/DevPanel/Evolution/Logs/Memory/Money/Onboarding/Performance/Portfolio/Scheduler/WorldMonitor 等，另有大量 `any` 类型债 |

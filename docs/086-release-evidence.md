...
| Codex `0.147.0` | 部分通过 | Apache WebSocket 握手从 426 修为 101；Sub2API 对 API Key 账号的 WS 调度仍返回无可用账号，未冒充客户端成功 |
...
| WebUI 深链刷新 | 观察 | 移动端 `/usage` 记录 1 次瞬时 503；业务 API 全部 200，随后同路由 12/12 为 200、Console 0 error/0 warning，按 HI-1006 观察 |

永久测试账号为用户 ID 2，必须保留；密码、API Key、Token 和 Cookie 只在钥匙串或服务器私有配置中，不出现在本报告。渠道A空分组、Codex WS 调度和源站公网绕过是仍未关闭的 P1，验收方案见 `docs/009-health.md` HI-1001、HI-1002、HI-1005。

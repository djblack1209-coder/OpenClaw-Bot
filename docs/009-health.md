...
| HI-990 | `BUSINESS` | 🟠 重要 | 库存阻塞 | 闲鱼/CC readiness v2 已按当前生产真值返回 12 个分组、12 个账号、12 条启用渠道、10 条启用文本监控与 2 条禁用生图监控；自动发货仍暂停且可用兑换码库存为 0，正式售卖门保持关闭。补库存、恢复 owner-loop 并完成真实小额同单验收前不得公开售卖。 |

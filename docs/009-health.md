...
| HI-1000 | `QA/PERF` | 🟡 一般 | 真实链路已验收 | 保留账号 `jiyu-e2e-20260808@245334.xyz` 已完成 Claude Code、OpenAI Responses、原始 Responses WebSocket 和 Codex `0.147.0` 真实调用并产生 10 条用量记录。最小 WS 样本缓存读取 3840 / 总输入 4395（87.37%），客户端首输出 4287 ms、总时长 4524 ms；真实 Codex 样本首 Token 2482 ms、服务端总时长 2964 ms、站内计费 `$0.087439`。账号、历史用量和活动 Key 保留，密码/Key/Token/Cookie 均不入仓库或截图。 |
...
| HI-1012 | `SECURITY/AI_POOL` | 🟡 一般 | 自营池已配置，真实号源待资产所有者操作 | Sub2 `v0.1.172` 原生不能直接解析卖家 `邮箱----密码----totp_secret`，只接收已生成的 auth/token 凭据。本地助手现严格兼容分隔行、标签块和 JSON 对象/数组，识别 Plus/Pro 后只匹配 `JIYU OpenAI Plus/Pro 自营号池`；生产已创建两个专属空池，Plus 模板 `2.0x`、Pro 模板 `3.0x`，空池首号自动继承模板，已有倍率不一致仍失败关闭。Telegram `/jiyu_replenish` 仅授权管理员私聊接收号源并复用本机 Runner；原文、密码、TOTP、Token 不落盘/日志/响应，2FA 只用本机 PyOTP。真实账号仍可能要求 CAPTCHA、短信或海外实体手机号，助手不会绕过。 |
...
| HI-1021 | `ARCH_LIMIT/OPS` | 🟡 一般 | 软件已接入，需本机持续运行 | Telegram 已增加远程补号菜单，闲鱼现有 `/xianyu start|stop|status` 继续作为本机 GUI/客服进程开关；两者只控制本机进程，不转发 Cookie、订单或浏览器会话。Bot 与 Web App 不可同时运行补号批次，资产所有者仍需在本机浏览器处理 OAuth 风控。 |

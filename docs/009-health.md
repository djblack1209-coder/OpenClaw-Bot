...
| HI-1012 | `SECURITY/AI_POOL` | 🟡 一般 | 本地工具已就绪，真实补号待人工挑战 | Sub2 `v0.1.172` 原生不能直接解析卖家 `邮箱----密码----totp_secret`，只接收已生成的 auth/token 凭据。本地助手现严格兼容分隔行、标签块和 JSON 对象/数组，批次只选一次渠道A/B（默认A），Plus/Pro 由 OAuth 元数据自动匹配；OTP 选择器不再泛匹配任意数字输入。原文、密码、TOTP、Token 不落盘/日志/响应，2FA 只用本机 PyOTP；聚焦测试保持 `11 passed`。真实账号仍可能要求 CAPTCHA、短信或海外实体手机号，助手不会绕过。 |
| HI-1013 | `ARCH_LIMIT/XIANYU` | 🟠 重要 | 候选已定，部署客观阻塞 | 老板已允许更重助手，选定 `zhinianboke/xianyu-auto-reply` 为独立 AGPL 服务候选；截至 2026-08-08 为 6,337 stars、最近 pushed 2026-08-01，功能与资源要求已登记。国内 VPS 仅约 13GB 空闲，源码构建可能突破磁盘安全余量；Oracle SGW 有约 88GB 空闲但海外出口对闲鱼登录风控不利。本轮不真实部署、不复制合并源码；部署前必须固定 commit、自构建、私网绑定、强随机凭据、关闭远程广告/公告/卡片远程基址并加反代认证。 |

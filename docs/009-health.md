...
| HI-985 | `DEPLOY/AI_POOL` | 🟡 一般 | 内测可运营，持续观测 | 渠道A与渠道B各 5 个 JIYU 专用 Key、10 个分组、10 个一对一渠道和 10 条监控已上线；全部用户倍率相对当前上游倍率绝对增加 `0.05x`，渠道A OpenAI Plus 为 `0.06x → 0.11x`。监控统一为 300 秒和 ±30 秒，页面如实并列显示正常、降级和错误，不用静态绿灯覆盖故障。 |
...
| HI-994 | `DEPLOY/ARCH_LIMIT` | 🟠 重要 | 方案待确认 | 官方 WebUI 自更新会安装上游二进制并覆盖 JIYU 代码级定制，之前还被 Apache 拦截成截断的 `status c`。当前 `v0.1.172-jiyu.5` 已消除同基础版虚假更新并保持安全失败关闭，但尚未达到老板要求的 WebUI 一键更新。推荐新增 CI 兼容包、SHA-256 清单和 root-only 安装代理；不要直接解禁官方更新接口。 |
| HI-995 | `AI_POOL/BUSINESS` | 🟠 重要 | 上游资料待确认 | 老板指出两个上游的生图能力未同步。源码确认 Sub2API 原生支持 Images API、独立权限和计价，但本机两套相关上游配置只登记文本模型，未发现可验证生图模型、价格或协议。需确认每条线路的真实模型、接口、并发和成本后，再建立匿名生图渠道；不得猜模型或擅自发起付费请求。 |
| HI-996 | `BUG/SECURITY` | 🟠 重要 | 教程已完成，运行时待重建 | CC Switch 数据库中两个旧生图 MCP 都指向不存在的本地脚本，现有入口无法启动。已形成官方 MCP SDK + JIYU `/v1/images/generations` 的安全 stdio 方案和小白提示词；待生图分组上线并创建专用 Key 后，重建 MCP、停用旧条目并仅做一次经确认的真实单图验收。 |
| HI-997 | `SECURITY` | 🟠 重要 | 待依赖分组审计 | 本次推送后 GitHub 默认分支报告 31 个 Dependabot 漏洞（1 critical、13 high、17 moderate）。远端只给出聚合数量，尚未证明均影响生产运行路径；需按运行时/开发时依赖分组、查看修复版本并分批升级，禁止为了清零数字直接改锁文件或升级生产底座。 |

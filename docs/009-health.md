...
| HI-1006 | `PERF` | 🔵 低优先 | 观察中，暂无新异常 | 真实浏览器曾记录 1 次 `/usage` 导航 503，随后同路由 12/12 为 200。08-08 源站 25 次 503 已全部归因：16 次为渠道A Claude 明确失败验证，9 次集中于发布重启窗口。当前兼容包于 2026-08-08 16:06:55 UTC 发布；截至 16:27:35 UTC 未出现 5xx，`sub2api.service` 的 `NRestarts=0`，公网健康连续 5/5 为 200，当前管理页 Console 0 error/0 warning。Codex 首次模型目录刷新曾瞬时失败，但随后 `/v1/models` 两种请求均为 200；不做猜测性改动。 |

...
| HI-994 | `DEPLOY/ARCH_LIMIT` | 🟠 重要 | 线上已关闭 | 旧 Apache 拦截曾把更新错误截成 `status c`。现生产已启用 CI 兼容包、`jiyu-latest` 移动清单、SHA-256/大小/架构校验、固定 root-only 代理、最小 sudoers、原子暂存与独立 systemd 哈希/健康验证。首个候选因未嵌入前端导致首页 404，被真实 GET 门发现并从发布前备份回滚；修正后 `v0.1.172-jiyu.31237926226` 首页/监控/健康均为 200，固定 sudo 路径返回“当前基础版已最新”且无截断错误。 |

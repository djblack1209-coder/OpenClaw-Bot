...
| HI-997 | `SECURITY` | 🟠 重要 | 推送回执降至 3 项，继续分组审计 | main 推送的 GitHub 回执从 31 项降至 9 项，再降至 3 项（2 high、1 moderate）；已合并 `h2 4.4.1`、`hpack 4.2.0`、`pypdf 6.15.0`，并将桌面构建的 `js-yaml` / `nanoid` 收口到 `4.3.1` / `3.3.17`。剩余项需按生产运行时/开发时确认，禁止为了清零数字直接升级生产底座。 |
...
| HI-999 | `CI/BUG` | 🟡 一般 | 本地已关闭，待远端复验 | 首次兼容包 CI 因补丁生成未纳入两个未跟踪的共享排序器新文件而类型检查失败；主 CI 同时发现生图安装器 `SC2155`、闲鱼预检 `F841`，以及桌面端 `js-yaml` / `nanoid` 两个新高危公告。补丁已包含新文件，lint 问题已修，传递依赖已提升到安全版本；只重跑对应门。 |

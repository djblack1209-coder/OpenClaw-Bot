...
| 编号 | 分类 | 严重度 | 状态 | 当前结论 |
|---|---|---|---|---|
| HI-983 | `DEPLOY/SECURITY` | 🟠 重要 | 线上已关闭 | 旧主站 New-API 运行库、上游 Key、用户/Token/渠道/日志和回滚副本已按老板要求清理；Oracle 主站当前为基于官方 Sub2API `v0.1.172` 固定提交构建的 `v0.1.172-jiyu.31265860057`，只绑定 loopback `18080`，PostgreSQL/专用 Redis 均为独立实例。受管 WebUI 更新只安装带 JIYU 补丁的校验包，发布带完整备份、健康检查和失败回滚。 |
...
| HI-991 | `PERF/QA` | 🔵 低优先 | 移动端满屏已关闭；全站矩阵仍待补 | 生产 `v0.1.172-jiyu.31265860057` 已把充值 shell 固定在 64px JIYU 顶栏下；真实 Chrome `390×844` 干净重载记录 iframe `x=0,y=64,w=390,h=780`，外层 `scrollWidth=clientWidth=390`，固定整店地址无查询参数，Console 警告/错误与失败网络请求均为 0。桌面 `1440×1000` 保持 `1176×936`。链动小铺自身横向商品滑动和紧凑导航属于第三方内部布局，不跨域修改；全站双主题、同一登录态全页面矩阵仍是历史审计缺口。 |

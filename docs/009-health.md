...
| HI-1006 | `PERF` | 🔵 低优先 | 观察中，暂无新异常 | 真实浏览器曾记录 1 次 `/usage` 导航 503，随后同路由 12/12 为 200。08-08 源站 25 次 503 已全部归因：16 次为渠道A Claude 明确失败验证，9 次集中于发布重启窗口。当前兼容包于 2026-08-08 16:06:55 UTC 发布；本次回读 `sub2api.service` 的 `NRestarts=1` 是 systemd 历史累计计数，服务仍为 `active/running` 且主进程退出码 0；精确 fatal/panic 与 JSON/键值 5xx 日志匹配均为 0，公网健康连续 5/5 为 200，账号 #2 为 active、可调度、单分组。Codex 首次模型目录刷新曾瞬时失败，但随后 `/v1/models` 两种请求均为 200；不做猜测性改动。 |

...
| HI-991 | `PERF/QA` | 🔵 低优先 | 本轮双视口证据已关闭，历史矩阵待补 | 文档下载区和充值中心均已在 1440×1000、390×844 生成真实改后截图。充值 iframe 分别为 `1176×936` 与 `390×780`，查询参数、Console 异常和失败请求均为零；移动端外层无横向页面溢出，专用外链兜底按钮已隐藏，不再遮挡第三方导航。链动小铺自身在 390px 内仍使用横向商品滑动并压缩顶部导航，这是第三方页面内布局，不会撑宽 JIYU 外层，需后续跟随链动上游改版复核。全站双主题、同一登录态全页面矩阵仍是历史审计缺口。 |

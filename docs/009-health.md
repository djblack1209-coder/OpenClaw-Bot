...
| HI-1001 | `AI_POOL/BUG` | 🟠 重要 | 根因已确认，上游线路待处理 | 渠道A Claude 官 Key 分组绑定账号 #2，账号为 `error`、关闭调度且无临时限流；最近错误明确为上游分组拒绝 `/v1/messages`。渠道B同协议成功。继续保持不可调度，避免假绿；验收仍是上游恢复该协议后通过真实 Claude Code，或经业务确认后在恢复前隐藏公开分组。 |
...
| HI-1008 | `FRONTEND/DEPLOY` | 🟠 重要 | 修复待发布 | 官方版本比较只认识 `v0.1.172`，因此生产 JIYU 旧修订与新兼容包同属一个基础版时 `hasUpdate=false`，版本面板不显示安装入口；root 代理也曾按基础版直接跳过。现 JIYU 构建始终显示受管“检查并安装”，代理改为比较完整构建号，已是最新时映射为官方 HTTP 200 无操作响应。验收标准是生产能从旧修订安装新修订，并在新修订再次点击时不下载、不重启、不报错。 |

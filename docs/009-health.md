...
| HI-1012 | `SECURITY/AI_POOL` | 🟡 一般 | 本地工具已就绪，自营池待业务配置 | Sub2 `v0.1.172` 原生不能直接解析卖家 `邮箱----密码----totp_secret`，只接收已生成的 auth/token 凭据。本地助手现严格兼容分隔行、标签块和 JSON 对象/数组，识别 Plus/Pro 后只匹配 `JIYU OpenAI Plus/Pro 自营号池`，不再把采购库存误导入渠道A/B；池不存在或不唯一时失败关闭。原文、密码、TOTP、Token 不落盘/日志/响应，2FA 只用本机 PyOTP。自营池的模板倍率、用户可见性和调度关系尚待业务确认，真实账号仍可能要求 CAPTCHA、短信或海外实体手机号，助手不会绕过。 |

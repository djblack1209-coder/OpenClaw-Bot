...
| HI-987 | `BUSINESS/COMPLIANCE` | 🟠 重要 | 商品已上架，充值专用构建待发布 | 保证金账户已真实显示 ¥100；7 档 JIYU 余额商品各导入 1 张既有兑换码并上架，公开页均可点击“立即购买”。旧 Markdown iframe 被清洗为 300×150 且移动端白屏，明确不作为闭环；固定公开店铺、无认证参数拼接的专用全内容区嵌入已进入候选补丁，仍需生产双视口验收。未执行真实购买；首笔 ¥1 付款、自动发货、兑换到账仍需操作当时再次确认。 |
...
| HI-991 | `PERF/QA` | 🔵 低优先 | 文档证据已补，充值证据待新构建 | 文档下载区已在 1440×1000、390×844 生成真实改后截图，未发现横向溢出、遮挡或下载入口尺寸漂移。充值中心必须等待专用构建上线后重新生成同尺寸截图并记录 iframe 尺寸、Console 和失败请求；全站双主题、同一登录态全页面矩阵仍是历史审计缺口。 |

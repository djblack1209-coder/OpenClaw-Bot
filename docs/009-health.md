...
| 编号 | 分类 | 严重度 | 状态 | 当前结论 |
|---|---|---|---|---|
| HI-1039 | `BUG/INFRA` | 🔴 阻塞 | 线上已关闭，外部告警待资产侧承接 | 2026-08-12 06:04:47 UTC，Ubuntu 自动更新重启 PostgreSQL/Redis 依赖时，Redis 因 `/etc/sub2api` 父目录被 Cloudflare 源站策略写入路径改回 `root:root 0750` 而无法读取自己的 `0640 root:sub2api-redis` 配置；Sub2API 依赖启动失败，Apache 从约 06:08 起持续返回 503，直到 10:22:07 UTC 恢复，服务不可用约 4 小时 17 分。恢复前确认最新日备份可由 `pg_restore --list` 解析，并保存管理器与 ACL prestate；随后只补两个运行用户的目录穿越 ACL、重启依赖并回读。管理器现统一由 `prepare_config_directory` 维护最小 ACL，Cloudflare/WebUI/安装路径不能再覆盖它；真实 Redis 重启后 Sub2API 于第 2 秒恢复，12 次公网 `/health` 均为 200、未授权 `/v1/models` 为 401。现有 VPS-Config 主机探针在故障期间仍返回成功，因为它不检查 JIYU；按项目边界未修改该外部控制面，外部告警仍需资产侧现有监控承接。 |
...
| 编号 | 分类 | 严重度 | 状态 | 当前结论 |
|---|---|---|---|---|
| HI-1023 | `AI_POOL/FRONTEND` | 🟠 重要 | 线上已关闭 | 模型广场原先使用渠道定价目录，未启用分组白名单，导致展示未实测的预置模型。已通过 Sub2 原生“同步上游模型”对 12 个账号做一次实测：11 个成功、1 个生图账号无法返回清单；生产 12 条渠道已保存“上游清单 ∩ 已有定价”的精确映射并启用 `restrict_models`，未验证渠道失败关闭。CI `31278104138` 已由生产 WebUI 检查并安装后重启生效，线上 VERSION 回读 `v0.1.172-jiyu.31278104138`；`/api/v1/model-plaza` 与页面均只呈现受限映射模型（11 个分组，模型数按组为 1/9/8/1/8/9/6/6/7/1/7），页面 Console 错误/警告为 0。 |
| HI-1024 | `XIANYU/OPS` | 🟡 一般 | 本机已关闭 | 闲鱼运营投影与两份运营摘要仍指向已移除的 JIYU `/api/health`，实际健康入口为 `/health`。2026-08-09 已统一更正并进行本机服务重载；不涉及生产数据库、支付、库存、账号或 Cookie。 |
| HI-1025 | `TECH_DEBT` | 🔵 低优先 | 本地已关闭 | 审计确认一个旧插件配置脚本、一个旧日志轮转配置和 20 张历史截图均无当前调用者；前两项已分别由现用卖家浏览器启动器和 `tools/newsyslog.d/openclaw.conf` 替代。删除前逐项确认 Git 可恢复，未触碰备份、迁移、支付、测试、生产证据或本地浏览器快照。 |
| HI-1026 | `DOCS/UX` | 🔵 低优先 | 本地已关闭 | 老板手册曾把“替换模式模拟验收”、状态导出和去掉 `--dry-run` 的恢复写入放入日常路径，可能使演练/诊断 JSON 被误当作业务完成，或误触服务重启/数据覆盖。2026-08-09 已收口为技术支持边界：老板只运行预演；任何带 `--confirm` 或会重启服务的恢复动作必须先确认最新状态、可恢复备份和最小影响范围。日常与恢复售卖仍以真实健康探针和新的真实小额订单链路为准。 |
| HI-1027 | `TEST/BUG` | 🟡 一般 | 本地已关闭 | 消息链路端到端测试夹具未定义补号协程，`MagicMock` 被生产代码 `await` 后抛出 `TypeError`，使普通中文命令分派从未执行。2026-08-09 已将夹具默认值更正为返回 `False` 的 `AsyncMock`；基线提交同样复现，未修改生产消息处理。 |
| HI-1028 | `SECURITY/OPS` | 🟠 重要 | 文档已收口，运行时待维护窗口 | 当前 Sonic 专用维护隔离窗口未结束期间，VPS 运维 Bot 的 `/scan`、`/repair`、`/manual` 不得发起 broad 或 Sonic 目标的 `vpsctl pull/status/diff`、探针或修复；缺少明确主机和最小范围时必须失败关闭。唯一当前基线和新会话提示词已补充该边界；运行时 prompt 的永久修复需等用户明确开启维护窗口后，由技术支持做目标化最小修改并按 fresh prestate、备份、回滚、真实业务探针闭环。 |
| HI-1029 | `AI_POOL/OPS` | 🟡 一般 | 待资产所有者补号 | JIYU Plus/Pro 两个自营号池的分组外壳与倍率模板均正常，但当前账号成员、活动渠道和监控均为 0。只读审计现明确标记为“库存待补”，并与公开售卖渠道就绪分开报告；不会因自营库存尚未导入而误报当前 12 条售卖渠道失败。 |
| HI-1031 | `SECURITY/AI_POOL` | 🟠 重要 | 本地代码已收口，外部响应未验证 | Telegram 远程敏感补号和本地批次控制均已禁用：`/jiyu_replenish` 只提示使用 `make jiyu-sub2-replenish`，并一次性保护消费下一条普通私聊文本，不读取内容。`status`、`stop` 及遗留键盘操作明确回复 Telegram 无法查看或控制本地批次，不构造 `ReplenishRunner`、不读取状态也不调用停止；仅 `cancel` 可取消保护等待状态。当前记录不保留敏感标识或内部定价公式。 |
| HI-1032 | `SECURITY/XIANYU` | 🟠 重要 | 本地代码已收口，保留运行态边界 | 旧 `make cc-seller-chrome` 入口及 `--copy-token` 参数已退出当前路径，遗留参数立即失败关闭，不会复制 Token。旧 QR API 路由、桌面封装、`qr_login.py` 与 Telegram QR 通知链已删除，因无生产调用者且可由 Git 恢复。当前请在 OpenClaw 桌面端点击“启动并打开运营台”，保持隔离卖家浏览器登录，由本机桥接器通过回环 CDP 接管闲鱼；CookieCloud 与运行态 Cookie 读取继续保留。仍未解决的边界是 `xianyu_live` 的 `.env` 自动浏览器登录/刷新链，必须在独立维护窗口确认其替代方案，禁止重新引入导出、剪贴板或聊天转发会话。 |
| HI-1033 | `SECURITY/FRONTEND` | 🟡 一般 | 本地代码已关闭 | 旧 New-API 兼容代理在未配置 `NEWAPI_BASE_URL` 时会隐式尝试连接本机 `localhost:3000`，可能把未启用的兼容链误接到无关服务；桌面端也未解包 v1 `{data:{items:[]}}` 列表，AI 配置统计占位项可显示为可启停。现已改为显式地址缺失即 503、所有代理路径不发起 HTTP 请求，桌面端兼容 v1 列表并将负 ID 统计项固定为不可操作。未删除 router、桌面入口、Compose 或迁移/恢复链路。 |
| HI-1034 | `SECURITY/XIANYU/UX` | 🟡 一般 | 本地代码已关闭 | OpenClaw 桌面端 Xianyu 页面现通过 `POST /api/session/desktop-launch` 获取仅回环、约 60 秒有效且只能消费一次的启动链接；消费后进入现有 15 分钟 HttpOnly 管理会话。`XIANYU_ADMIN_URL` 会在读取 API Token 前限制为无路径/凭据的本机回环 HTTP 基地址，远程、HTTPS、伪造后缀主机、路径/查询和缺失主机均失败关闭，端口仍可配置。API Token 只在本机请求头中使用，不进 URL、响应或日志。旧 Telegram 二维码登录卡已移除，登录继续由隔离卖家 Chromium 完成；本轮未启动服务、未写生产。 |
| HI-1035 | `BUG/INFRA` | 🟠 重要 | 本地代码已收口，真实下一次构建待观察 | 一次真实 `make tauri-build` 在构建过程中曾使 `/Applications/OpenClaw.app` 暂时缺失，随后已从严格签名的当前构建工件与既有回滚副本恢复；当时未捕获到可归因的中断原因。安装器现保留已安装 App，直至候选包完成构建、严格签名和临时副本校验，再同目录交换；失败 trap 会恢复本地备份和持久回滚副本。已移除跨 worktree 删除和 Dock/Launchpad 重置。未以再次安装代替验证；SIGKILL、断电或文件系统故障发生在两次 rename 之间仍只能依赖同目录 `.previous-*` 暂存与持久回滚副本人工恢复，需在下一次明确维护窗口观察。 |
| HI-1036 | `SECURITY/XIANYU/UX` | 🟡 一般 | 本地代码已关闭，首次登录仍需用户 | 闲鱼桌面入口现在只启动独立卖家浏览器并通过回环 CDP 由本机桥接器接管，不再要求 Social Pilot 扩展、`chrome://extensions`、手动命令或 Token 本地复制；状态投影也不再要求刷新插件或执行重启命令。操作台明确提示在 OpenClaw 桌面端点击启动并首次登录。X/小红书继续使用独立 Social Pilot 能力，不受闲鱼入口改动影响。 |
| HI-1037 | `SECURITY/XIANYU/UX` | 🟡 一般 | 本地代码已收口，待桌面构建观察 | 闲鱼桌面页的“启动并打开运营台”现在先复用现有 Tauri 托管服务控制器幂等启动 `ai.openclaw.xianyu`，再启动隔离卖家浏览器并交换一次性回环地址；服务已运行时不重启。未新增依赖、常驻服务、凭据输入或控制面；真实桌面点击和打包 App 复验仍待明确维护窗口。 |
...
| 前沿 | 状态 | 证据 / 下一步 |
|---|---|---|
| 软件审计发现 | 已闭环 | HI-965 至 HI-978 均有代码、聚焦测试或实机证据；当前生产状态以 `docs/current/current-baseline.md` 为准。 |
...
| 编号 | 分类 | 严重度 | 状态 | 当前结论 |
|---|---|---|---|---|
| HI-983 | `DEPLOY/SECURITY` | 🟠 重要 | 线上已关闭 | 旧主站 New-API 运行库、上游 Key、用户/Token/渠道/日志和回滚副本已按老板要求清理；Oracle 主站当前为基于官方 Sub2API `v0.1.172` 固定提交构建的 `v0.1.172-jiyu.31278104138`，只绑定 loopback `18080`，PostgreSQL/专用 Redis 均为独立实例。受管 WebUI 更新只安装带 JIYU 补丁的校验包，发布带完整备份、健康检查和失败回滚。 |
...
| HI-985 | `DEPLOY/AI_POOL` | 🟡 一般 | 内测可运营，持续观测 | 渠道A与渠道B各 5 个文本专用 Key，加上 2 个生图专用账号；12 个分组和 12 个一对一渠道已建立。10 条文本监控按 300±30 秒运行；2 条生图监控保存同样合同和加密凭据，但因上游 502/401 保持禁用。已批准的用户定价由所有者控制，公开文档不暴露公式或单价。 |
...
| HI-987 | `BUSINESS/COMPLIANCE` | 🟠 重要 | 商品与充值入口已闭环，实单待确认 | 保证金账户已真实显示 ¥100；7 档 JIYU 余额商品各导入 1 张既有兑换码并上架，公开页均可点击“立即购买”。旧 300×150 Markdown iframe 已由固定公开店铺、无认证参数拼接的专用全内容区嵌入替代；生产双视口确认七档商品可见且无白屏。未执行真实购买；首笔 ¥1 付款、自动发货、兑换到账仍需操作当时再次确认。 |
...
| HI-990 | `BUSINESS` | 🟠 重要 | 闲鱼历史门已闭环，链动实单待确认 | 闲鱼/CC readiness v2 当前可用兑换码为 7、自动发货未暂停、历史真实订单 1 单、补救队列 0；默认与 `--require-real-order` 两种只读审计均为 PASS。该历史闲鱼订单只证明闲鱼链路严格门，不证明链动首笔 ¥1 购买、自动发货和站内兑换；链动实单仍须操作时单独确认。 |
| HI-991 | `PERF/QA` | 🔵 低优先 | 移动端满屏已关闭；全站矩阵仍待补 | 生产 `v0.1.172-jiyu.31271410817` 已把充值 shell 固定在 64px JIYU 顶栏下；真实 Chrome `390×844` 干净重载记录 iframe `x=0,y=64,w=390,h=780`，外层 `scrollWidth=clientWidth=390`，固定整店地址无查询参数，Console 警告/错误与失败网络请求均为 0。桌面 `1440×1000` 保持 `1176×936`。链动小铺自身横向商品滑动和紧凑导航属于第三方内部布局，不跨域修改；全站双主题、同一登录态全页面矩阵仍是历史审计缺口。 |
| HI-992 | `SECURITY` | 🟠 重要 | iframe 精确放行已关闭，其余策略待收紧 | 生产 CSP 保持 nonce、`frame-ancestors 'none'` 和 `form-action 'self'`；链动整店只通过原生 `security.csp.policy` 在 `frame-src` 精确增加 `https://pay.ldxp.cn`，未放宽脚本或连接来源。`connect-src https:` 与 `img-src https:` 仍允许任意 HTTPS，且 Permissions-Policy 仍待补；收紧前需盘点支付、验证码、对象存储和 OAuth 真实域名，采用 report-only 观察后再切强制策略。 |
...
| HI-1000 | `QA/PERF` | 🟡 一般 | 真实链路已验收 | 保留测试账户已完成 Claude Code、OpenAI Responses、原始 Responses WebSocket 和 Codex `0.147.0` 真实调用并产生 10 条用量记录。最小 WS 样本缓存读取 3840 / 总输入 4395（87.37%），客户端首输出 4287 ms、总时长 4524 ms；真实 Codex 样本首 Token 2482 ms、服务端总时长 2964 ms、站内计费 `$0.087439`。账号、历史用量和活动 Key 保留，密码/Key/Token/Cookie 均不入仓库或截图。 |
| HI-1001 | `AI_POOL/BUG` | 🟠 重要 | 兼容透传已恢复，定价待业务确认 | 渠道A Claude 官 Key 账号 #2 开启仅替换认证的自动透传，自动探测开启、自动同步关闭；默认旧模型仍返回“不支持”，改用账号白名单内 `claude-sonnet-4-6` 后单次真实测试获得有效响应，随后才恢复正常状态与调度。上游声明倍率为 `0.45x`，账号计费/用户分组仍为 `0.12x/0.17x`；本轮按“不改定价”边界保留，需业务确认后另行调整。 |
...
| HI-1004 | `SECURITY/INFRA` | 🟡 一般 | 基础链与 Passkey 已关闭，Turnstile 待凭据 | JIYU 主机保持严格 TLS、Managed WAF、OWASP、L7 DDoS、主机级注册/验证码与登录/2FA 限流；Passkey 已使用固定 RP ID 和 HTTPS 来源启用，忘记密码可用。开放注册仍关闭、邮箱验证开启；Turnstile 没有专用 Site Key/Secret，继续关闭，获取真实凭据并完成双视口注册/登录验证前不得开放注册。 |
| HI-1005 | `SECURITY/ARCH_LIMIT` | 🟠 重要 | JIYU 443 线上已关闭 | JIYU 443 使用 Cloudflare Origin CA，独立 nftables 表只允许 Cloudflare 官方 CIDR、loopback 和 Tailscale。应用前安排 5 分钟自动回滚；公网三个 vhost 正常后，三个独立外部 VPS 直连源站均超时才取消回滚。共享 80 端口同时承载 `naive-iad` vhost，并存在 active `naive-cert-renew.timer`；其续期依赖尚未证明可安全移除。JIYU 自身 80 只返回 301，443 已闭环；不得以 JIYU 名义修改或关闭共享 80。 |
| HI-1006 | `PERF` | 🔵 低优先 | 观察中，暂无新异常 | 真实浏览器曾记录 1 次 `/usage` 导航 503，随后同路由 12/12 为 200。08-08 源站 25 次 503 已全部归因：16 次为渠道A Claude 明确失败验证，9 次集中于发布重启窗口。2026-08-09 重新取证：`v0.1.172-jiyu.31278104138` 服务重启后 `NRestarts=0`、主进程退出码 0，应用错误日志与 `ops_error_logs` 中的新增记录均为 0，公网 `/health`、模型广场接口和页面均为 200。永久测试账号保留 10 条真实用量，其中 WS 2 条；累计缓存读取 20608/输入 21660（95.14%），平均首 Token 5913 ms、总时长 12203 ms，站内计费 `$0.132916`、上游成本 `$0.052531`。不为制造样本发起循环或付费请求。 |
...
| HI-1011 | `SECURITY/OPS` | 🟡 一般 | 外部凭据阻塞 | LinuxDo、GitHub/Google 邮箱快捷登录、Turnstile、支付服务商和异步生图 S3 均缺少真实专用凭据，因此保持关闭；异步生图不会复用现有备份存储。Sub2 原生支付支持 EasyPay、支付宝、微信支付和 Stripe，但必须配置真实商户实例、签名凭据和 HTTPS 回调。链动整店只是兑换码售卖页，未提供可信回调前不得自动给余额入账。生产管理员 API Key 仅经不回显管道写入 macOS 钥匙串，未写入仓库、文档或截图。 |
| HI-1012 | `SECURITY/AI_POOL` | 🟡 一般 | 自营池已配置，真实号源待资产所有者操作 | Sub2 `v0.1.172` 原生不能直接解析卖家 `邮箱----密码----totp_secret`，只接收已生成的 auth/token 凭据。本地助手现严格兼容分隔行、标签块和 JSON 对象/数组，识别 Plus/Pro 后只匹配 `JIYU OpenAI Plus/Pro 自营号池`；生产已创建两个专属空池，Plus 模板 `2.0x`、Pro 模板 `3.0x`，空池首号自动继承模板，已有倍率不一致仍失败关闭。全部材料只进入 localhost UI；Telegram 仅提供安全提示、一次性保护消费和取消，真实号源始终由资产所有者操作。原文、密码、TOTP、Token 不落盘/日志/响应，2FA 只用本机 PyOTP。真实账号仍可能要求 CAPTCHA、短信或海外实体手机号，助手不会绕过。 |
| HI-1013 | `ARCH_LIMIT/XIANYU` | 🟠 重要 | 本机 GUI 可用，外部候选待试运行 | 现有 `http://127.0.0.1:18800/dashboard` 已是本机图形化运营台，负责商品、订单、库存、补救和严格门真值。社区中 `PerfectWorld233/XianyuAgentPro` 提供 Electron GUI 但为 GPL-3.0，`GuDong2003/xianyu-auto-reply-fix` 为活跃 AGPL-3.0 候选；二者只能独立试运行，不能合并进现有闭环或覆盖库存/发货真值。平台登录、CAPTCHA 和风控仍必须由资产所有者处理。 |
| HI-1014 | `AI_POOL/ARCH_LIMIT` | 🟠 重要 | 原生探测已核对，自动写回禁用 | Sub2 `v0.1.172` 原生 upstream billing probe 全局已启用，当前周期 30 分钟，可设 5-1440 分钟。探测只读取上游 `/v1/sub2api/billing`，仅受支持 API Key 上游可返回结果；12 个账号的 rate sync 均关闭。写回只修改 `accounts.rate_multiplier`，不会同步用户分组，因而无法保证“用户分组倍率 = 账号倍率 + 0.05x”。直接启用会破坏定价一致性，因此在业务确认定价策略和原子账号/分组联动方案前禁止开启；生图的每张 `+0.05` 继续独立处理。 |
| HI-1015 | `BUG/OPS` | 🟡 一般 | 本地已关闭 | `cc_zhongzhuan_readiness_audit.mjs` 曾写死旧 10×10、后又写死 12 分组口径，会把新增的 Plus/Pro 自营号池误算成可售文本分组，导致真实生产审计误报倍率合同失败；同时旧 webhook 路由被移除后，安全的 404 被误判为未防护。现合同明确为“12 个可售分组（10 文本 + 2 生图）+ 2 个 OpenAI 自营号池”，校验自营池 Plus `2.0x`/Pro `3.0x`、文本差 `0.05x`、生图按次 `0.10/0.12`、10 条文本监控启用、2 条生图监控禁用、全部 300±30 秒。无认证 webhook 的 401/403/404 均为失败关闭。聚焦回归和默认只读实跑均 PASS，未改生产状态。 |
...
| HI-1017 | `FRONTEND/BUG` | 🟡 一般 | 线上已关闭 | API 密钥页端点说明浮层已在兼容包 `v0.1.172-jiyu.31271410817` 发布并通过 WebUI 更新；真实 Chrome `390×844` 创建密钥弹窗回读 `body.scrollWidth=390`、`documentElement.scrollWidth=390`。 |
...
| HI-1020 | `ARCH_LIMIT/OPS` | 🟡 一般 | 只读复核通过，持续观察 | Apache `/v1/responses` upgrade 规则在通用根代理之前，近期公网 Codex 请求有 `101` 握手；Sub2API 模式路由全局开启，四个 OpenAI 文本账号为 `http_bridge=true`，两个生图账号为 `off=false`。Responses 用量 6 条、其中 WS 2 条，最新样本含缓存读取 1408、首 Token 2482 ms、总时长 2964 ms、计费 `$0.087439`；未循环发起付费请求。Cloudflare 443 收口服务 active，nftables 仅允许官方 CIDR、loopback 和 Tailscale，本次回读服务 `NRestarts=1` 为历史累计重启计数，当前 `active/running` 且 `ExecMainStatus=0`，本轮未改防火墙。 |
| HI-1021 | `ARCH_LIMIT/OPS` | 🟡 一般 | 历史菜单已收口，本机 UI 持续运行 | 历史 Telegram 补号菜单不能控制 Web UI 所属的真实 Runner，当前只提供本机 UI 提示、取消保护等待和一次性保护消费。闲鱼现有 `/xianyu start|stop|status` 继续作为本机 GUI/客服进程开关；均不转发 Cookie、订单或浏览器会话，资产所有者仍需在本机浏览器处理 OAuth 风控。 |
...
当前结论：本节是 2026-08-04 的历史整改记录。当前生产状态、未修复项和优化优先级只以 `docs/current/current-baseline.md` 为准。
...
| **综合** | **8.4** | `(9+9+8+8+8)/5`；历史评分，不作为当前生产事实 | 评分只覆盖当时的 macOS 单机 + Oracle 内测拓扑 |
...
2026-07-08 追加 GitHub 轮子调研：当时按用户提供的 21 个仓库完成一次调研；实现完成后该快照报告因无生产调用者于 2026-08-11 退役，详细版本可从 Git 恢复。真实结论仍是不直接复制许可证或安全边界不清晰的闲鱼自动化、抢拍/强聊或 OSINT 代码。
...
2026-07-07 最新收口（历史记录）：系统已补齐三条发货路径：1）WebSocket 结构化付款卡片自动发卡；2）卖家订单列表 `NOT_SHIP` 轮询兜底（当前登录态可能返回 `PERMISSION_EXCEPTION::无权限访问`，只能作为加分能力）；3）当时的 Chrome 付款页 `paid_page_dispatch` 兜底，在当前闲鱼页可见“已付款/待发货”时自动生成话术并发送。确认发货已支持后端真实数字订单号内存确认和浏览器页面点击两种路径；恢复可售已补 `/api/cc-xianyu-relist/next` 队列，只对已确认发货记录候选，且页面必须显示已下架/已售罄和重新上架按钮才点击。严格门口径已收紧：只有 `xy_oid_*` 真实自动订单才算正式售卖证明，`xy_manual_*` 与 `xy_browser_*` 只算内测/补救，不再冒充真实自动订单；看板历史摘要也已按该口径规整，旧手工单不会再显示为真实自动订单。当前只读生产内测巡检 `ok=true`，但 `--require-real-order` 严格门 `ok=false`，原因是尚未产生新的 `xy_oid_*` 真实闲鱼付款自动发货订单。该段最后一次实机复核记录的是旧社媒插件状态和 `make cc-seller-chrome` 加载方式；当前入口已迁移为 OpenClaw 桌面端点击“启动并打开运营台”，保持隔离卖家浏览器登录，由本机桥接器通过回环 CDP 接管闲鱼，旧命令和扩展目录不再是现行操作。
...
| ID | 分类 | 描述 | 发现日期 | 状态 |
|----|------|------|----------|------|
| HI-1038 | INFRA/LEGAL | JIYU 登录前条款与文档源曾错误声明中国大陆整体不开放。2026-08-09 已在生产条款和文档源中改为“中国大陆 IP 仅国内模型、境外 IP 全部模型、海外主系统唯一账本”，并明确技术分流尚待真实回读；Sub2API、Redis、Apache 与回环健康均通过。Cloudflare 地区过滤、服务端目录过滤以及大陆/境外真实页面/API 回读尚未完成，不能宣称规则已实施。 | 2026-08-09 | 🔴 P0 技术分流待实施；法律文案已修正 |
...
## 63. JIYU 闲鱼八档发布与首单观察收口（2026-08-11）

生产浏览器先出现两套 1/10/30/50/100/300/500/1000 元 JIYU 商品。经用户明确指定保留本轮新建组后，逐项核对旧商品面额和旧文案并删除旧组；个人页回读确认旧组全部消失，新组八档各 1 个，没有按 807 张兑换码库存展开发布。

写入前运行现有一致性备份；自动发货保持暂停时，通过受保护 API 写入 8 条真实商品映射，并在 SQLite 单事务中删除唯一旧测试映射。旧 `xianyu-test-1` 默认兜底同时置空，未知商品不再可能误领任意面额。服务重启后真实回读：无鉴权接口 401、WebSocket/Cookie 正常、映射 8/8、Sub2API 可用兑换码 807、补救队列 0、恢复预检 `safe_to_resume=true` 且无阻断。

随后开启首单观察，运行态为自动发货开启、观察票剩余 1 次、可处理真实已付款订单、补救队列 0。系统只会在真实付款信号后原子预留对应面额兑换码，首条消息成功后自动暂停；本轮没有制造订单、分配兑换码或发送测试消息。新八档的首笔自然订单尚未发生，因此支付、发码和买家兑换只记录为待真实业务回读，不提前宣称通过。

...
2. 用户提交充值单后，让用户按充值单金额付款，不要让用户自己改金额。
3. 用户付款备注写注册邮箱或订单号，避免同金额订单无法区分。
4. 管理员核对收款二维码后台记录、充值单金额和用户邮箱。
5. 核对无误后在管理页人工确认入账。
6. 每天导出或截图保存一份收款流水，后续对账时按订单号、邮箱、金额和时间倒查。
...
1. 用企业或个体工商户主体注册支付宝开放平台账号。
2. 创建网页/扫码支付应用，申请开通支付宝当面付。
3. 拿到商户号、应用 AppID、支付宝网关地址。
4. 生成应用私钥并上传应用公钥，保存支付宝平台公钥。
5. 在应用里配置异步通知地址，建议预留为 `https://你的域名/api/frist/payments/alipay/notify`。
6. 确认签名算法、字符集、金额单位和订单号长度。
7. 把 AppID、商户号、应用私钥、支付宝平台公钥和签名密钥写进服务器环境文件。
...
1. 注册微信支付商户平台，完成主体实名和经营类目审核。
2. 绑定公众号、小程序或 AppID；没有现成 AppID 时先确认微信支付后台允许的产品形态。
3. 开通 Native 支付产品。
4. 获取商户号、AppID、商户 API 证书序列号、APIv3 密钥、商户私钥。
5. 配置异步通知地址，建议预留为 `https://你的域名/api/frist/payments/wechat/native/notify`。
6. 确认回调解密方式、签名密钥、订单号规则、金额单位和回调重试规则。
7. 把证书、私钥和 APIv3 密钥放到服务器安全路径，不要放进 Git。

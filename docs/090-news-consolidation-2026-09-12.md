...
新备份仅保存在本机专用目录，既有每日与异地备份任务不变。改动前后分别保存归档并执行 checksum、路径、manifest 和 SQLite 只读恢复演练；前备份 25 项、后备份 24 项 inventory warning 为可选路径缺失或跳过，不等同于完整灾备恢复。另保存 Git bundle、运行配置、原 LaunchAgent 前态和缺失依赖文件的恢复清单；含私有数据的归档及证据不纳入 Git。

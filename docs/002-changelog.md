...
> 影响模块: `Sub2API groups`, `profit control`, `channel monitor`, `compatibility package`
> 关联问题: JIYU-20260816-MARKETING-GO-LIVE
### 变更内容
- 按运营策略修正，不再以上游稳定性关闭文本产品：通过原生分组表单恢复此前停用的 12 个分组，最终 14 个成本型公开文本组全部 active。创建 Key 的真实下拉框显示 14 项，空占位组和生图组未混入。
- 完整清点并逐份阅读官方仓库 46 个 Markdown 文档，以及 v0.1.173/v0.1.177 的部署配置和相关原生实现。复用官方 `upstream_billing_probe`、倍率同步和利润控制，不新增成本 daemon 或控制面。
- 14 个成本型文本组统一启用原生 `profit_min_margin=0.03`、`profit_safety_buffer=0`；14/14 当前账号通过准入，最低配置毛利 `3.7037%`。渠道 A 五组自动倍率探测 `ok/200`，渠道 B 五组与国内四组为供应商接口 `unsupported/404`。
- 10 个文本监控全部恢复，2 个专用生图监控保持关闭。自然样本显示 6 正常、2 降级、2 上游错误；同机其他供应商正常且本方 DNS/TLS/route、数据库和 Redis 错误为 0，因此不再关闭错误/降级文本分组。
- 定位到监控表格粘性操作列覆盖开关，点击会命中“复制”。现有 v0.1.173 兼容补丁复用 DataTable 原生 `stickyActionsColumn`，仅在监控页关闭粘性操作列；生产坐标回读确认开关与复制按钮不重叠。
- 兼容包工作流 `31947794554` 从官方 v0.1.173 应用全部现有补丁、完成前端 typecheck/build、聚焦 Go 测试和 ARM64 构建并发布不可变工件。生产受管发布到 `v0.1.173-jiyu.31947794554`，独立运行哈希与健康验证结果为 `applied`。
### 纠错与回滚
- 首轮跨平台表单操作因数字字段顺序差异，曾把 10 个分组的 `image_price_1k` 从 0 临时写为 3。回读立即发现并停止；保留中间态备份后，10/10 恢复为 0，并按字段标签正确设置 3% 毛利率，最终其余价格字段无漂移。
- 表格覆盖验证期间两次误触“复制”，只产生停用副本；每次均在备份后删除，原监控及历史未丢失，最终重复记录为 0。审计保留完整创建/删除轨迹。
- 关键回滚点为分组前 `daily-20260816T122257Z`、监控前 `daily-20260816T123608Z`、发布前 `daily-20260816T124932Z`；最终一致性备份为 `daily-20260816T125208Z`。本机归档 `openeverything-20260816-210241.tgz` 的 checksum、路径、manifest 和 SQLite 只读恢复演练通过，离机目标仍未配置。
### 验证
- 发布后管理器状态、PostgreSQL、Redis、Cloudflare 443、内网健康和 Responses WebSocket 全部通过；systemd failed unit 0，订单 pending/processing 0，用量记录保持 10。
- 真实管理员页显示版本 `v0.1.173-jiyu.31947794554`、12 条监控定义，操作列为 static 且 `overlap=false`；用户 `/monitor` 显示全部 10 个文本渠道，专用生图不展示。
- 历史“只开放渠道 A OpenAI Plus/Pro”的目录收敛已被本条运营策略明确取代；该记录保留为真实历史，不再作为当前基线。

## [2026-08-16] 营销目录收敛与成本口径纠正
> 时间: `2026-08-16T11:24Z`
> 领域: `ai-pool` | `infra` | `docs`
> 影响模块: `Sub2API 费率`, `公开分组`, `渠道监控`, `营销放行`
> 关联问题: JIYU-20260816-MARKETING-READINESS-CLOSE
### 变更内容
- 纠正上一条营销前审计的账本口径：`actual_cost` 是用户实际扣费，`total_cost` 是倍率前基准价，账号侧预估成本按 `total_cost × account_rate_multiplier` 计算。近 30 天 10 笔真实用量的用户扣费 `$0.05253097`、账号侧预估成本 `$0.04588517`、预估毛利 `$0.00664580`，负毛利 0 笔；原先 4 条“亏损”是字段比较错误，不是生产亏损。
- 在一致性备份 `/var/backups/sub2api/daily-20260816T112114Z` 与最新 prestate 后，通过原生分组管理只把 12 个未同时满足“成本自动回读 + 当前稳定可用”的公开分组改为 inactive，保留账号、渠道、费率、绑定、模型和历史数据。回滚为按 prestate 将这 12 个分组恢复 active。
- 当前可售目录只保留渠道A OpenAI Plus/Pro。两组均为唯一活动账号、无用户专属倍率/RPM 覆盖、利润控制开启、账号费率同步开启、成本探测 fresh `ok`，分组倍率分别为 `0.12/0.15`，均比账号倍率高 `0.05`。
### 验证
- 审计日志回读到 12 条 `admin.groups.update`/HTTP 200，数据库为 active 2、inactive 12、disabled 生图 2；服务、Redis、PostgreSQL 和内网 `/health` 均正常，写入后订单与用量均为 0。
- 真实 Chrome 用户创建 Key 下拉框只显示渠道A OpenAI Plus/Pro，没有创建 Key。既有调度器继续检查两项保留产品，并按受管绑定失效保护自然停用已下线产品监控；最终为 2 条启用、10 条停用，自 10:45Z 起两项保留产品各 9/9 次 operational，没有人工触发模型探测。
- 收敛后新增 Oracle 一致性备份 `/var/backups/sub2api/daily-20260816T113037Z` 和本机归档 `openeverything-20260816-193034.tgz`；本机归档通过 checksum、路径、manifest 与 SQLite 只读恢复演练，离机备份未配置的现状未被掩盖。

## [2026-08-16] 营销前费率与成本审计
> 时间: `2026-08-16T11:02Z`
> 领域: `ai-pool` | `infra` | `docs`
> 影响模块: `Sub2API 费率同步`, `公开分组`, `渠道监控`, `营销放行`
> 关联问题: JIYU-20260816-MARKETING-COST-PREFLIGHT
### 变更内容
- 只读回读确认 14 个公开分组均有唯一活动账号，没有用户专属费率或 RPM 覆盖，所有当前分组倍率都高于关联账号成本至少 `0.05`；8 个符合文本同步条件的分组与“账号倍率 + 0.05”合同一致，漂移为 0。
- 自动成本来源并非全覆盖：10 个启用费率同步的活动账号中，5 个账单探测为 fresh `ok`、5 个为 `unsupported`。渠道B的 Claude/OpenAI 四个公开分组因此无法随上游成本自动重定价；两个 Grok 组因允许生图被文本费率同步有意排除。国内四个 OpenAI 分组当前关闭利润控制，但仍有静态正价差且没有用户覆盖价。
- 近 30 天仅有 10 条真实计费记录，全部发生在 2026-08-08；其中渠道B Claude 官 Key 的 `claude-sonnet-4-6` 有 4 条实际成本高于用户计费，累计亏损 `$0.00725970`，最大单条 `$0.00374325`。之后没有新真实计费样本，不能用当前静态合同声称该历史路径已验证修复。
- 没有修改费率、分组、账号、监控、支付或营销入口。全量拉新暂不放行，等待运营明确选择收缩可售目录，或接受静态成本并指定人工复核责任与亏损阈值。
### 验证
- 10 条启用监控均在 10 分钟内完成检查；当前 3 条 Claude 为 `error`、2 条 Grok 为 `degraded`，其余为 `operational`。这说明不能把总健康端点等同于可对外承诺的产品可用性。
- 生产结论来自 PostgreSQL 的费率、绑定、用户覆盖、账本和监控只读回读；未制造模型负载、订单、支付或合成计费请求。

## [2026-08-16] 渠道A OpenAI Pro 分组与监控恢复
> 时间: `2026-08-16T10:45Z`
> 领域: `infra` | `docs`
> 影响模块: `Sub2API V1 渠道监控`, `渠道A OpenAI Pro`, `当前生产基线`
> 关联问题: JIYU-20260816-CHANNEL-A-PRO-MONITOR-RECOVERY
### 变更内容
- 生产回读确认目标监控被停用的直接前提是绑定的 OpenAI Pro 分组处于 `disabled`，而对应渠道和账号均为 `active`。受管监控的通用凭据解析器会在绑定失效时禁用监控并取消调度；该保护不按渠道显示名称硬编码。
- 在最新一致性备份和仅含状态的 prestate 通过后，使用原生分组管理界面只将该分组恢复为“正常”，不改账号绑定、倍率、模型或支付配置。恢复途中发现一条额外的停用重复监控记录；再次生成当前备份/prestate 后，事务只删除该重复项、保留原记录并启用。
- 通过原生监控编辑保存路径重新登记唯一启用记录的调度任务；未点击“立即检测”或手工构造探针，调度器按既有首次检查语义完成实际检查，不重启全局服务或新增控制面。
### 验证
- 数据库回读为目标分组、渠道和账号均 `active`，监控记录恰好 1 条且 `enabled=true`；恢复后的实际检查已更新，随后日志中没有新的“绑定失效”自动停用事件。
- 原生后台列表显示开关已启用；公网 `/health` 继续返回 200。生产结论来自这些运行时回读，不以本地测试数量替代。
...
> 影响模块: `Sub2API 充值菜单`, `openclaw-sub2api-manager`, `链动小铺`, `云猫寄售`
> 关联问题: OPENCLAW-20260816-REMOTE-MANAGER-DRIFT
### 变更内容
- 最终生产回读发现 Oracle 的受管管理脚本未包含已提交的菜单标签参数化，导致数据库仍保留旧标签；远端与仓库 HEAD 的完整 diff 仅为两项标签常量及其 SQL 参数引用。
- 在受保护的远端脚本 prestate 和校验和之后原子同步当前脚本；若后续菜单部署失败，事务会恢复旧脚本。随后只调用既有 `recharge-center`，不修改支付、订单、渠道、模型或二进制。
- 回读确认“链动小铺”和“云猫寄售”各有一项菜单记录，旧标签不存在；两个自定义页仍为 HTTP 200，浏览器回读的 iframe 不带 query/hash 会话参数。
### 验证
- 远端脚本语法、校验和、PostgreSQL 菜单布尔回读、Sub2API/Redis/备份 timer/Cloudflare 源站策略状态和公网端点均通过；失败路径保留时间戳脚本与菜单 prestate。
- 本机严格健康、OpenClaw health/doctor、窄屏 Chrome 布局回读和 Mac 已安装 App 验收通过；未制造业务负载或读取凭据。

## [2026-08-16] 充值入口命名与云猫自配支付只读核对
> 领域: `frontend` | `deploy` | `docs`
> 影响模块: `链动小铺`, `云猫寄售`, `Sub2API 充值菜单`, `云猫自配支付`
> 关联问题: OPENCLAW-20260816-RECHARGE-LABELS-PAYMENT-AUDIT
### 变更内容
- 将左侧两个充值入口的显示标签从“充值中心”和“充值中心2”统一改为“链动小铺”和“云猫寄售”；页面 ID、公开店铺地址、全屏嵌入和 CSP 边界保持不变。
- 通过现有 Chrome 登录态只读核对云猫“自配支付渠道”：能力已开通，但“易支付支付宝”和“易支付微信”均关闭且均无收款账号；新增账号字段为别名、易支付网关、PID、密钥。
- 明确收款归属：云猫表单接收的是易支付服务商商户接入凭据，最终结算账号由服务商侧绑定的支付宝/微信商户决定；本轮未添加账号、未开启支付、未提交资质或产生订单。
### 验证
- `bash -n scripts/sub2api_oracle_manage.sh`、`node --test scripts/sub2api_ops_scripts.test.mjs` 和 `git diff --check` 通过。
- 生产写入仅限菜单显示标签，执行前使用既有 `recharge-center` prestate/数据库备份/失败回滚/健康与页面探针；支付通道保持原关闭状态。

## [2026-08-16] 云猫七档与充值中心2生产闭环
> 领域: `frontend` | `deploy` | `infra` | `docs`
> 影响模块: `云猫寄售`, `充值中心2`, `JIYU Sub2API`, `GitHub Actions`
> 关联问题: OPENCLAW-20260815-YUNMAO-RECHARGE2-CLOSE
### 变更内容
- 通过云猫原生跨站搬家完成链动小铺复制：分类 `1/1`、商品 `7/7`、库存 `707/707`；后台七档均为“销售中”，每档 101 张，公开店铺七档均显示“库存充足”。
- 店铺资料、公告、官网按钮和已有联系方式已按链动可见资料同步；默认手续费承担方真实回读为“买家承担”。
- 首次生产启用 `充值中心2` 时发现旧 JIYU 前端会给外部 iframe 拼接站内用户查询参数；已用最新菜单 prestate 立即移除入口并保持服务健康。随后以同一官方 `v0.1.173` 的修复工件 `v0.1.173-jiyu.31894209937` 发布，重新启用菜单。
- 生产写入均经过日备份、二进制发布前备份、菜单/CSP/page prestate、最小变更、真实健康探针和自动回滚保护；最终 CSP 的链动与云猫来源各出现一次，云猫只保留在 `frame-src`。
- GitHub Actions 运行 `31894209937` 全部通过；定时任务对官方 `v0.1.176` 等未审阅补丁安全跳过，手动指定未审阅版本仍失败关闭。
### 验证
- Oracle `sub2api.service`、PostgreSQL、专用 Redis、Apache、更新 timer、备份 timer 和 Cloudflare 443 源站策略均 active；内外 `/health` 为 200，未授权 `/v1/models` 为 401，两个自定义页面为 200。
- 真实 Chrome 登录态确认左侧 `充值中心2`、云猫 iframe 无 query/hash，且与链动页 iframe 的整屏尺寸一致；没有读取或提交密码、Token、Cookie、私钥、订阅地址或账户标识。
- 生产发布前备份 `/var/backups/sub2api/jiyu-v0.1.173-jiyu.31894209937-20260815T161855Z` 与菜单安全回滚 prestate 均可恢复；本地静态/合同检查通过，生产结论不以本地测试数量替代。

## [2026-08-15] 云猫店铺资料与七档商品模板准备
> 领域: `frontend` | `deploy` | `docs`
> 影响模块: `云猫寄售`, `充值中心2`, `链动小铺库存迁移`
> 关联问题: OPENCLAW-20260815-YUNMAO-INVENTORY-HANDOFF
### 变更内容
- 使用现有 Chrome 登录态，把链动小铺可见的店铺昵称、公告、官网按钮和已有联系方式同步到云猫；云猫手续费承担方保存为买家承担。
- 在云猫新建 `JIYU AI 余额` 分类及 `¥1/10/50/100/300/500/1000` 七档 `JIYU AI 余额兑换码` 商品模板，补齐与链动一致的自动交付、余额兑换和使用说明；七档均保持下架，避免零库存误售。
- 云猫原生跨站搬家检测链动源站成功，迁移范围可包含商品、描述、图片与未售库存；由于平台要求提交源站账号密码，未代用户提交凭据、未导入卡密正文、未改变链动库存。
- Oracle 未执行 `recharge-center` 写入；`充值中心2` 仍需等库存迁移完成后再按 prestate、备份、最小变更、真实探针和失败回滚顺序部署。
### 验证
- 云猫后台真实页面显示 7 个商品，价格与标题均为 `1/10/50/100/300/500/1000`，状态为“已下架”、库存为 0；资料保存回读显示昵称/公告/官网按钮存在且手续费为买家承担。
- 云猫迁移检测返回“检测成功：新版本卡网”；未点击“开始迁移”，未提交密码、Cookie、Token 或卡密正文。
...
- 收口充值部署安全边界：执行前把数据库、配置、页面和当前菜单保存为带时间戳的 prestate；任一 SQL、CSP、重启、健康或页面探针失败都会自动恢复并再次健康检查，回滚失败则保留备份路径供人工恢复。
...
> 关联问题: OPENCLAW-20260815-PRODUCTION-FINAL-AUDIT
### 变更内容
- 完成 Mac OpenClaw、Oracle/JIYU、备份校验和真实 Chrome 登录态的只读生产复查；没有生产写入、版本升级、模型负载、订单或支付操作。
...
> 关联问题: JIYU-20260811-XIANYU-OFFICIAL-APPLICATION
### 变更内容
- 使用现有 Chrome 登录态进入闲鱼三方开放平台，确认当前账号没有可管理应用；打开“申请注册”后预填小程序名称、业务说明和默认行业，企业淘宝昵称留空，未提交申请。
- CleanMyMac 误关页面后重新打开官方申请表并恢复相同预填内容；同时只读核对当前普通卖家网页版发布页，确认只有一个商品价格输入框，没有 SKU/多面额入口，“一口价”是发货设置中的运费方式。
- 新增可直接用于 AppKey/小程序审批的 MRD/PRD、官方准入步骤、最小权限、聚石塔/奇门数据边界、退款状态和发布验收材料；不新建第二库存、订单事实源或空壳适配器。
- 修正旧生图/充值计划中“闲鱼尚未发布、仍使用旧 SQLite 卡池”的过时事实，恢复与八档已上架、Sub2API 原子预留和首单观察一致。
### 验证
- 申请表回读为名称和业务说明校验通过、默认行业已选、淘宝 nick 为空；没有点击“确定”，没有发送合同、身份、账号或凭据。
- 普通卖家个人页回读为八档各一个独立商品；网页版发布页没有原生多规格或自动发卡配置入口。移动端未连接实体设备，因此不把移动端能力推断为已验证。
- 资产目录未发现企业淘宝、企业支付宝、营业执照或 ICP 主体资料；淘宝开放平台企业入驻页受浏览器安全策略阻止自动控制，明确保留为人工登录、实名和授权步骤。

## [2026-08-11] Channel Monitor 恢复 V1 与闲鱼官方履约边界确认
> 领域: `ai-pool` | `infra` | `docs` | `xianyu`
> 影响模块: `Sub2API Channel Monitor`, `闲鱼虚拟订单`, `当前生产基线`
> 关联问题: JIYU-20260811-MONITOR-XIANYU-OFFICIAL
### 变更内容
- 生成现有 Oracle 一致性备份后，仅把 `channel_monitor_mode` 从 `v2` 切回 `v1`；V2 配置、水位、聚合表和历史完整保留，未改变 10 条启用监控定义。
- 明确闲鱼开放平台的小程序/虚拟订单能力为运营定向邀请制，并非个人卖家可公开自助申请；官方逐单虚拟发货 API 只标记发货，不负责分配或展示卡密。
- 收敛后续设计：获邀后由官方订单消息和订单查询验证付款，通过聚石塔/奇门把最小订单映射交给现有 Sub2API 原子预留，再在获批的小程序页面/消息展示卡密并调用官方虚拟发货；Sub2API 继续作为唯一库存和账本事实源。
### 验证
- Sub2API 重启后内外 `/health`、服务状态和 10 条监控定义正常；V1 历史恢复写入且 V2 水位停止推进，没有制造模型请求、订单、支付或消息外发。
- 官方快速接入、服务端 API、发布集成、聚石塔/奇门和协议文档完成交叉核对；在运营邀请、同主体企业账号/合同、C/B AppKey 与发布审核完成前，不创建无生产调用者的官方适配器。

## [2026-08-10] JIYU 私有 R2、原生异步生图与闲鱼履约阻断复核
> 领域: `ai-pool` | `infra` | `docs` | `xianyu`
> 影响模块: `Sub2API 数据备份`, `异步生图`, `JIYU 生图 MCP`, `闲鱼自动发货`
> 关联问题: JIYU-20260810-R2-ASYNC-XIANYU
### 变更内容
- 复用现有 Cloudflare 资产创建 JIYU 专用私有 R2 bucket 和限定桶读写凭据；数据库备份使用 `backups/`，应用保留 7 天/最多 8 份且 R2 以 8 天兜底清理；图片使用 `images/` 并在 2 天后删除。未开放公网访问、未升级套餐。
- Sub2API WebUI 启用每日 03:30 原生备份和异步生图对象存储；首份 R2 备份下载、SHA-256、gzip SQL 和隔离恢复通过，共恢复 96 张表，临时数据库与文件已清理。
- 中央 `JIYU AI 生图` MCP 切换到 Sub2API 原生异步提交/轮询端点，专用 Key 仅存 macOS 钥匙串并设置 1 USD 总额度；真实 SDK 客户端可发现工具，任务被 202 接受后如实回读上游失败，没有产生费用或图片对象。
- 两条用户提供的上游均使用最小只读/单图探针且未保存凭据：渠道 A 权限拒绝，渠道 B 可列出模型但不支持目标图片模型。停止重复探测，等待供应商恢复。
- 闲鱼文件上传权限已解除，但生产已付款 webhook 的遗留 Frist-API 卡池可分配库存为 0，无法领取 Sub2API PostgreSQL 的 807 张八档兑换码。为避免付款后无卡可发，未发布商品、未解除自动发货暂停，也未复制明文卡密形成第二事实源。
### 验证
- R2 私有访问、生命周期、WebUI 双连接测试、首份备份对象回读和隔离恢复通过；中央 MCP 语法、安装状态和真实 JSON-RPC 工具调用通过。
- 本机闲鱼 WebSocket、Cookie、补救队列和历史严格门正常；当前公开售卖资格因库存事实源分裂明确失败关闭，需先迁移受管 webhook 再做真实 ¥1 闭环。
...
> 关联问题: HI-983, HI-991
### 变更内容
- 将当前生产版本摘要统一为 `v0.1.172-jiyu.31271410817`；保留旧构建作为历史证据并明确时间口径。
- 删除交接文档最旧的第六条记录，保持只保留最近 5 条；未改变任何生产业务配置或脚本行为。
### 文件变更
- `docs/007-operations.md` — 更新充值中心当前构建摘要。
- `docs/009-health.md` — 更新当前版本健康摘要，保留历史发布证据。
- `docs/012-handoff.md` — 清理过期交接并标明旧构建为历史状态。
...
> 关联问题: HI-1012, HI-1013, HI-1016, HI-1017
### 变更内容
...
- 真实 Chrome 复核充值、API 密钥/创建密钥、文档下载、渠道状态和系统设置；充值页未出现失败请求或运行时异常。系统设置加载的 CSP report-only 控制台噪声和首屏延迟保留在后续风险中，不以绿灯掩盖。

## [2026-08-09] JIYU 生产只读审计合同升级为 12×12 双轨
> 领域: `infra` | `ai-pool` | `docs`
> 影响模块: `JIYU readiness audit`, `系统健康`, `会话交接`, `命令注册表`
> 关联问题: HI-985, HI-990, HI-1001, HI-1006, HI-1015
### 变更内容
- 将 Oracle 只读审计从过期的 10 个分组、10 个渠道、10 条启用监控合同升级为当前 12 个分组、12 个账号、12 条启用渠道和 12 条监控合同；只检查启用渠道的一对一分组关系，并要求 12 个账号均只有一个业务分组。
...
### 文件变更
- `docs/001-project-map.md` — 更新 JIYU 版本、Codex WS 和 443 拓扑事实。
- `docs/007-operations.md`、`docs/009-health.md` — 同步倍率探测、稳定性证据和 80/443 运维边界。
- `docs/012-handoff.md` — 新增当前交接并移除最旧条目。
- `docs/002-changelog.md` — 记录本次纯文档同步。
### 验证
- `git diff --check` 与 `make docs-check` 通过；未修改代码、生产、定价、支付、数据库、注册策略或上游线路。

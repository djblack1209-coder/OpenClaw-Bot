...
> 最后更新：2026-07-12。旧的 `start_all.sh`、`stop_all.sh`、`pack_deploy_bundle.sh` 和旧部署打包入口已经删除，不得从历史记录恢复执行。

### 当前边界

- 闲鱼链路默认 `read_only_watch`；订单状态未知、Cookie/页面异常或没有一次性确认票时必须暂停。
- 模型、浏览器插件和桥接器不能自行确认“已付款”、发送卡密或点击确认发货。
- 模拟/替换模式只证明流程可演练，不能解锁正式售卖，也不能伪造真实订单号。
- 真实发卡、确认发货、恢复自动发货、账号登录、验证码/MFA 和生产服务重启都需要对应授权。
...
- 两个脚本使用当前项目路径和 `.venv312`，不会 `pkill -f` 其他进程；端口或实例已存在时会安全停止重复启动。
- 本项目不再提供“一键停止全部进程”脚本。前台运行使用 `Ctrl-C`；由 LaunchAgent/Docker 管理的实例按 `docs/007-operations.md` 对应服务步骤处理，禁止模糊杀进程。

### Cookie、凭据和打包

- Cookie、Token、Key、卡密和账号信息只放在本机被 Git 忽略的运行配置中，不粘贴到命令、文档、日志或聊天。
- 旧百度网盘 License 打包流程已停用；损坏、无引用的旧打包脚本已删除。
- 桌面 App 如需构建/安装，先取得授权，只能运行 `make tauri-build`；本轮审计未安装应用。
...
老板日常只看 `docs/081-owner-ops-handbook.md`；生产内测与故障处理以 `docs/007-operations.md` 顶部当前口径为准。
